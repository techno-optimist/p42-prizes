...
def test_rotation_rejects_predecessor_owner_key_as_successor_holder(tmp_path: Path) -> None:
    ...
    with pytest.raises(LaunchAuthorityRosterError, match="Ed25519 key|public_key is already"):
        normalize_launch_authority_roster(
            successor,
            trust_registry=registry,
            predecessor=predecessor,
...
def test_rotation_rejects_principal_identity_crossing_owner_boundary(
    ...
    if direction == "owner-to-holder":
        ...
        fresh_security = fixture.identity(
            "identity-owner-now-security-key", "Riley Security", "independent-security-authority"
        )
        prior_owner_identity = {
            **fresh_security,
            **_identity_fields(prior_signers[OWNER_ROLE]),
        }
        _appoint_holder(
            successor,
            registry,
        ...
        role_signers[OWNER_ROLE] = fresh_owner
        role_signers["independent-security-authority"] = prior_owner_identity
    else:
    ...
    with pytest.raises(LaunchAuthorityRosterError, match="principal name|identity is already"):
        normalize_launch_authority_roster(
            successor,
            trust_registry=registry,
            predecessor=predecessor,

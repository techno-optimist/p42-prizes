...
| 4 | `da_expiry_or_missing_payload` | commit w/o DA hash; finalize w/o permanence receipt | `commit(_, 0)` reverts `P42_EMPTY_DA_HASH`; on the deployed `3121a1a` bytecode, `finalize(_, 0)` reverted `P42_EMPTY_PERMANENCE_HASH`; DA hash bound into the on-chain `p42:v1` commitment. **Superseded at HEAD:** the DA refactor **removed** the finalize permanence gate — `finalize` now accepts `permanenceHash = bytes32(0)` (optional mirror record); DA integrity is instead enforced at reveal (`sha256(bytes) == commitDaHash` for `onchainDa=true` problems; the same anchor gates off-chain bytes). This attack row is evidence for the old deployment only and must be re-run against a redeployed post-refactor contract | `A.commit_zero_da`, `B.finalize_zero_permanence` reverted live (old bytecode); HEAD regressions: `contracts/test/p42-da.test.js` |
...
| `da_bound_at_commit_and_finalize` | ✅ on `3121a1a` bytecode only — **invariant changed at HEAD**: DA is bound at commit and enforced at **reveal** (`sha256(bytes) == commitDaHash`); finalize `permanenceHash` is optional (`bytes32(0)` accepted) | attack 4 live reverts (old gate); HEAD: `contracts/test/p42-da.test.js` |

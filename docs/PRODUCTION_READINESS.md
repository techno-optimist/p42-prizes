...
- No complete/deployed on-chain system: still missing the canonical Base Sepolia redeploy of the DA-refactored contracts (the BaseScan-verified deployment predates the refactor), a production indexer service, broader fuzzing/formal review, audit, and a non-owner-trusted resolver slashing path.
...
- Long-horizon availability past L1 blob pruning (~18 days) rests on a single-trust-domain calldata archive (`agent/indexer.mjs --archive`); an independent funded permanence mirror (e.g. Arweave) is defense-in-depth to add at real-ETH scale, not a launch blocker. For the 3 off-chain-DA problems, unavailable-payload policing is an operator challenge, not an automatic slash.

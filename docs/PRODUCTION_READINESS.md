...
- `docs/VERIFIER_RUNNER.md` defines DGX CHRONOS/Hermes as the immediate reveal verifier, transcript publisher, and alert/auto-challenge candidate while keeping runner output outside the settlement trust root; `p42-prizes runner-plan` adds an executable queue/OOM guard for verifier starts.
...
- No live DGX/Hermes reveal watcher yet; the runbook and local queue/OOM admission check exist, but event subscriptions, sandboxed verifier execution, queue persistence, transcript storage, alerting, and auto-challenge key/spend policy are not wired.

...
wrong, non-equivocating resolver decision. This is source and local-test
evidence only. It does not make a board production-ready until that board's
proof program and the immutable verifier gateway have passed independent
review, deployment rehearsal, proving-cost measurement, and external audit.
...
- an objective package hash committing to chain ID, registry, problem ID,
  specification, verifier source, verifier image, admission matrix, and proof
  program ID; and
- the proof program ID accepted for that board.
...
copies each program ID at construction, and immutably binds the verifier gateway
...
each board's proof-program bytes; deployment preflight reads those artifacts
from the trusted evidence root and rejects substitutions before signing. The
gateway runtime codehash must equal `keccak256(artifact.deployedBytecode)`, and
each objective program ID must equal `keccak256(exact program bytes)`. These
relations connect reviewed artifacts directly to the identities used on-chain.
...
instance, frozen objective package and program, pending transcript/verdict and
...
- Select, audit, deploy, and bytecode-pin the real verifier gateway. The mock
  gateway under `contracts/src/mocks` is test-only.
...
- Include the exact gateway, program IDs, package hashes, and rehearsal evidence
...
or v1 production release slate. During this predeployment gate phase those
...
program bytes, and package bindings required under the existing identifiers;

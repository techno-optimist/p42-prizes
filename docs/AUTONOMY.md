...
| **Container sandbox** for untrusted verifier payloads | ◑ `src/p42_prizes/runner_sandbox.py` defines a fail-closed Docker profile (no network, cgroup memory+PID+CPU caps, read-only rootfs, all caps dropped, non-root, read-only solution mount) with unit coverage. `deployments/base-sepolia/verifier-images.json` is historical local evidence and predates the current self-containment and supply-chain changes, so it cannot attest the current images. Before funding: rebuild and run all ten current images on trusted hosts, publish registry-pullable digests, collect the N-host matrix, and complete the deployed event-to-transcript rehearsal. |

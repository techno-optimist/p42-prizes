...
- Funding target publication is deadline-, authorization-, and cap-aware and fail-closed. The chain read model binds manifest and replay `fundingCapWei`, derives exact remaining capacity from projected `accountedBalanceWei`, binds projected authorization expiry and open state to the finalized on-chain observation, permits authorization-expiry equality exactly as Solidity, and suppresses every public pool/donation destination when expiry has passed or capacity is zero. The dynamic same-origin `no-store` `funding-target/v2` endpoint independently rechecks the cap equation and finalized checkpoint timestamp while exposing exact `remainingCapWei`; that value is an observation, not a reservation, so concurrent funding can still win the cap race. The page and React Server Component payload contain no target bytes at or after any portal cutoff, and focus/resume revalidation removes a cached target before requesting replacement authority. Focused read-model, API, and component regressions cover expiry equality and +1 second, zero cap remainder, missing/drifted fields, and stale UI revalidation. This is source evidence only and does not implement or close legal-policy acknowledgement. The live July 16 release intentionally returns `target: null` because no production activation packet exists.

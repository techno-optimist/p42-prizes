...
| --- | --- | --- | --- |
| Gate 0: Public repo / local pilot | Mostly green; the last live source/deploy check is historical, the integration candidate is not deployed, the production receipt is blocked on v3 ratification, and two repository-account controls remain | Local verifier, web/API tests, fail-closed challenge/onramp, historical live fail-closed PostgreSQL authority, security policy text, published `.github/workflows/ci.yml`, and a schema-validated historical v2 receipt. The exact historical evidence and current authority gap are recorded in the GitHub Actions row below; neither artifact authorizes the integration candidate. | Complete exact-head CI and a fresh release guard after merge/deploy; establish the externally pinned and threshold-ratified v3 source-release chain; retain a successful push run and release-guard record for each newly merged source release; upgrade the private repository/account tier or make the repository public so protected branch/ruleset enforcement is available; repo owner enables private vulnerability reporting through a supported GitHub surface |
...
| Portal state/rate limits/idempotency/events | Live state authority passed; checkpoint owner-function upgrade pending production provisioning | Historical live-state evidence remains `docs/evidence/portal-db-cutover-2026-07-13.json`. Current source confines checkpoint changes to a pinned migration-owner `SECURITY DEFINER` transition function and serves an exact unchanged checkpoint through a separately pinned, read-only `FOR SHARE` function using indexed maxima; a miss rolls back before fresh authority attestation and serialized transition. Runtime receives only `SELECT` plus those exact function `EXECUTE` grants, with direct DML, `TRUNCATE`, dangerous `SET ROLE`, external triggers, and authority drift rejected. Startup rejects runtime role, database, exact `pg_db_role_setting` default, effective search-path, or membership drift present at preflight before migration/ACL mutation; application pools explicitly use `<schema>,pg_catalog,pg_temp`, keeping temporary relations behind portal and catalog objects. That no-side-effect guarantee applies only to an initially hostile graph. A concurrent provider superuser or role administrator can change membership after preflight; final verification can then reject after the runner's migration transactions and ACL changes have committed, so an exclusive operator window is mandatory. Local PostgreSQL 18.4 evidence covers 26 migration/tamper cases, including session options masking a hostile role default, a fresh hostile graph with no persisted migration/ACL effects, runtime drift rejection, temp-shadow-resistant schema-pinned runtime queries, contiguous block/time regression, and nonadjacent full-identity replay; the separate concurrency rehearsal covers stale-transition contention and a 10,000-acceptance/six-reader run that observes all six reads in flight, proves zero mutual reader blockers, requires the serialized transition to be blocked only by held-reader PIDs, and enforces a 2,000 ms post-barrier completion ceiling. Authorization expiry is checked at the chain-bound finalized activation timestamp, while checkpoint freshness remains wall-clock bounded. This is source/local rehearsal evidence, not proof that Render roles, schema pin, or migration 002 are live. Migration 002 is new relative to base `e083f94`; unpublished intermediate draft schemas were never pushed or deployed and are not production compatibility inputs. | Preprovision the pinned Render schema and distinct owner/runtime roles under an exclusive operator window, inspect the role graph, apply the reviewed migration, retain redacted OID/function/ACL/privilege evidence and production rehearsal JSON, remove the owner credential from the web child, then verify live source and `target: null` before any later activation |
| Security disclosure text | Pass in repo | `SECURITY.md` | Repo owner must enable GitHub private vulnerability reporting |
| GitHub Actions workflow | Historical source/deploy pass; integration exact-head CI and live release guard pending; production receipt blocked on external v3 ratification | `.github/workflows/ci.yml` is published. Historical [`main` run 29533825184](https://github.com/techno-optimist/p42-prizes/actions/runs/29533825184) passed all seven required jobs for exact merged source head `5a4d25d3862d75d2b573e93f33f60ad72662aef7`: Python verifier, autonomous agent, portal, contracts, Ubuntu 22.04 and 24.04 SP1 rebuild/replay producers, and the cross-image `SP1 objective-program reproducibility` comparator. The authenticated Render guard independently bound branch head, runtime commit, and live commit to that same historical SHA, bound board projection `sha256:ffa222755590d96016bc0e1b151327ac912d4916a7c762d7c84f738e886365c2`, and reported 12/12 healthy routes. The later integration candidate is not covered by that run or live guard. Live funding publication remains deliberately unactivated (`target: null`). The prior [`source-release receipt`](evidence/source-release-current.json) remains valid historical v2 evidence for `f6dba23d8d76d4ff833077242397ef71727368db`, not current authorization. V3 source validation requires the exact ordered seven-job set plus PR or threshold-bootstrap authorization for every non-exempt first-parent source commit; it recursively replays predecessor provenance, authenticates historical/current PR rows, keeps Render relevance separate, and rejects approvals submitted at or after merge. Six producer jobs alone cannot authorize a v3 receipt | Complete the integration candidate's exact-head seven-job CI, then after merge/deploy retain a fresh release guard. Provision the externally pinned source-release v3 policy/trust root, independently review and threshold-sign every required migration interval, and publish the chained v3 receipt with pinned workflow identity and fresh live probes. Retain a completed push run and release-guard record for every source release. A July 16 recheck still returns GitHub `403` for branch protection/rulesets and `404` for private vulnerability reporting; those remain explicit repo-owner/account blockers |
...
- [x] Source-level operator and resolver runtime templates use distinct unprivileged accounts, a bounded `--once` supervisor with finite failure budgets, timeout escalation, private atomic self-hashed journals, an account-separated failure sink, explicit operator sandbox staging, systemd `LoadCredential` custody for signing and RPC URLs, and `LimitCORE=0`. Their complete `ExecStart` commands share the production CLI contract enforced by the executables, with no RPC URL or rootful Docker socket in process arguments; legacy receipt/endpoints/RPC environment fallbacks fail closed. The operator requires the reviewed same-user rootless Docker daemon, subordinate-ID/userns prerequisites, a private socket, and a pre-poll socket check; the worker additionally requires structured daemon identity, `name=rootless`, and `CgroupDriver=systemd`. The rootless unit's installed preflight rejects overlapping subordinate-ID ranges and runs a real same-user `unshare` probe; the sysusers fragment is documented as account creation only. The CI-called systemd aggregate checks both runtime units plus the rootless daemon/config/preflight, exact expanded-unit fixture tests exercise credential/staging/socket/RPC publication preflight, and Python source tests verify the worker binds staged payloads, passes the rootless endpoint to Docker, refuses pre-lease outages, and preserves per-container memory/PID/OOM/core controls. The unsigned [July 18 DGX operator rehearsal](../deployments/dgx-rootless-runtime-rehearsal/2026-07-18/REPORT.md) binds the installed bytes, passed native unit verification, observed rootless Docker 29.2.1 with the systemd cgroup driver, read back 64 MiB/32 PID container limits, observed an intentional OOM kill, removed all test containers, and restored rootful Docker while CHRONOS remained active. This closes target-host feasibility only. Independent signed runtime evidence, live RPCs, immutable verifier images, a real queue item, durable publication, signer provisioning, queue latency under load, and an on-chain poll remain open.
...
- [ ] SP1 dependency security remains an activation blocker. GitHub identifies the pinned v6.1 dependency closure's prerelease `p3-challenger` as affected by a high-severity advisory, with lower-severity `p3-symmetric` and `lru` findings also open. The current official SP1 v6.3.1 line does not clear all three findings and would change every frozen ELF, vkey, toolchain observation, and proof artifact. Do not hide the alerts or perform an evidence-destroying version bump. Objective-proof activation requires an exact reachability review plus an externally audited patched upstream release or maintained fork, followed by new versioned guests, dual-host reproduction, genuine proofs, economics, and independent replay. The read-only portal and native-verifier-only unfunded surfaces remain separable from this blocked proof tier.
...
- Funding target publication is deadline-, authorization-, cap-, and policy-acknowledgement-aware and fail-closed. The chain read model binds manifest and replay `fundingCapWei`, derives exact remaining capacity from projected `accountedBalanceWei`, binds projected authorization expiry and open state to the finalized on-chain observation, permits authorization-expiry equality exactly as Solidity, and suppresses every public pool/donation destination when expiry has passed or capacity is zero. The dynamic same-origin `no-store` `funding-target/v3` endpoint independently rechecks the cap equation and finalized checkpoint timestamp while exposing exact nonzero `fundingAuthorizationDigest`, `activationCompletionDigest`, checkpoint block, activation-finalization block, and `remainingCapWei`; that value is an observation, not a reservation, so concurrent funding can still win the cap race. The client strictly parses and click-time revalidates the exact authority binding, displays abbreviated digests, requires an accessible explicit acknowledgement, resets acknowledgement on binding or focus/resume revalidation, and preserves the separate second trusted click before wallet launch. The page and React Server Component payload contain no target bytes at or after any portal cutoff. Hostile regressions cover expiry equality and +1 second, zero cap remainder, missing/changed/zero/malformed digests, checkpoint drift, acknowledgement reset, stale UI/controller generations, and exact launch binding. This is source evidence only. The portal does not claim a browser live `eth_call`: its current evidence boundary is the fresh finalized dual-RPC indexer checkpoint, and a reviewed live dual-RPC pre-sign read remains a deployment gate. The live July 16 release intentionally returns `target: null` because no production activation packet exists.

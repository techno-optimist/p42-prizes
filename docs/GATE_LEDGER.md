...
- [ ] Forced-inclusion/censorship fallback is source-complete and locally tested: the L1 `P42ForcedInclusionController` is immutably restricted to the canonical portal, wallet, challenge manager, and challenge selector; its L2 alias occupies a one-time wallet role separate from governance ownership; forced execution still requires exact calldata, one-call policy, chain/expiry binding, exact deposited value, and shared spend caps; and `agent/censorship-fallback.mjs` refuses deadlines without two sequential sequencing windows plus safety. Gate remains open pending external review, canonical L1/L2 deployment, live Base configuration binding, and a signed Base Sepolia two-deposit rehearsal with canonical L1/L2 receipts (`docs/CENSORSHIP_FALLBACK.md`).

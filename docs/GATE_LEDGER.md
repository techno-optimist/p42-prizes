...
| GitHub Actions workflow | Live source and deploy pass; receipt reseal in progress | `.github/workflows/ci.yml` is published. [`main` run 29385762495](https://github.com/techno-optimist/p42-prizes/actions/runs/29385762495) passed all six lanes for exact source head `f81bb45406957109717d3cfbbc8b44158bc44f73`, including Ubuntu 22.04 and 24.04 SP1 rebuild/replay jobs. The authenticated Render record binds live deploy `dep-d9bfisl7vvec73a5p2ag` to that same deploy-relevant commit, and `make verify-render-release` passed all 10 committed route/configuration/projection checks. The prior [`source-release receipt`](evidence/source-release-current.json) remains a valid historical receipt for `f6dba23d8d76d4ff833077242397ef71727368db` but is intentionally not presented as current after the direct-pushed `/intro` changes. This evidence-only PR establishes a reviewed main-line anchor; a subsequent receipt must bind its exact merge commit, successful six-lane main CI, the current Render deployment, and fresh live probes before this row returns to full pass | Merge the reviewed evidence anchor, obtain successful CI on its exact merge commit, and publish a newly sealed receipt. Retain a completed push run and release-guard record for every source release. GitHub returned `403` for branch protection/rulesets and `404` for private vulnerability reporting; those are explicit repo-owner/account blockers |

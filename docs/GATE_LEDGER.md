...
- [ ] The source-level permissionless objective-proof path is structurally wired and locally tests package/program/gateway/runtime bindings plus both settlement outcomes, but the production SP1 gateway is immutably inactive and the launch-authority validator rejects any release that reports `objectiveProofsActive=false`. A P42-owned exact-32-byte guest, independently reproduced ELF-to-vkey derivation, active audited gateway, real per-board proof programs, economic sizing, deployment, and adversarial testnet rehearsal remain required before this gate can close or funding can be authorized.

...
- [x] Source-level permissionless objective proof can correct a non-equivocating resolver verdict and slash its bond; package/program/gateway/runtime bindings and both settlement outcomes are locally tested and indexer-reconciled. Real per-board proof programs, gateway audit/deployment, economic sizing, and adversarial testnet rehearsal remain open.

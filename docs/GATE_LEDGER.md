...
- Gate 2 custody/governance evidence now has an executable artifact path: `docs/CUSTODY_GOVERNANCE.md`, `schemas/governance-signoff-v2.schema.json`, and `p42-prizes governance-signoff-validate` require the exact ordered 47-contract Base Sepolia release binding, named multisig signers, strict-majority threshold, timelock, guardian limits, custody constraints, key-rotation rehearsal, recusal policy, governance/security-owner attestation, and canonical `governance_hash`. Historical `schemas/governance-signoff.schema.json` v1 remains non-authorizing.

...
| Portal state/rate limits/idempotency/events | Live state authority passed; checkpoint owner-function upgrade pending production provisioning | Historical live-state evidence remains `docs/evidence/portal-db-cutover-2026-07-13.json`. Current source confines checkpoint changes to a pinned migration-owner `SECURITY DEFINER` transition function and serves an exact unchanged checkpoint through a separately pinned, read-only `FOR SHARE` function using indexed maxima; a miss rolls back before fresh authority attestation and serialized transition. Runtime receives only `SELECT` plus those exact function `EXECUTE` grants, with direct DML, `TRUNCATE`, dangerous `SET ROLE`, external triggers, and authority drift rejected. Local PostgreSQL 16 evidence covers 18 migration/tamper cases, including contiguous block/time regression and nonadjacent full-identity replay, plus stale-transition contention and a 10,000-acceptance/six-reader exact-read rehearsal with zero blocking PIDs (21 ms measured batch maximum in the July 16 local run). Authorization expiry is checked at the chain-bound finalized activation timestamp, while checkpoint freshness remains wall-clock bounded. This is source/local rehearsal evidence, not proof that Render roles, schema pin, or migration 002 are live. | Preprovision the pinned Render schema and distinct owner/runtime roles, inspect the role graph, apply the reviewed migration, retain redacted OID/function/ACL/privilege evidence and production rehearsal JSON, remove the owner credential from the web child, then verify live source and `target: null` before any later activation |

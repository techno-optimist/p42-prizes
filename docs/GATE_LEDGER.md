...
- [x] Source-level operator and resolver runtime templates use distinct unprivileged accounts, a bounded `--once` supervisor with finite failure budgets, timeout escalation, private atomic self-hashed journals, and an account-separated failure sink. Their complete `ExecStart` commands now share the production CLI contract enforced by the executables, including agent-wallet custody, immutable operator provisioning, resolver quorum paths, explicit Arweave publication, and two retrieval endpoints. Shared-DGX source defaults add per-role `MemoryHigh`, `MemoryMax`, zero swap, and `TasksMax` ceilings without a CPU quota. The CI-called systemd aggregate checks both units, and agent fixtures complete one supervised poll against stub RPC/publisher behavior. This is local source/static and credential-free smoke evidence only: native installed-unit verification, DGX cgroup placement, live independent RPCs, funded durable publication, signer provisioning, queue latency under load, and an on-chain poll remain deployment evidence.

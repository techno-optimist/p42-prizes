...
| Portal state/rate limits/idempotency/events | Live state authority passed; checkpoint owner-function upgrade pending production provisioning | Historical live-state evidence remains `docs/evidence/portal-db-cutover-2026-07-13.json`. Current source moves all checkpoint writes into a pinned migration-owner `SECURITY DEFINER` function with `search_path=pg_catalog`; runtime receives only `SELECT` plus exact function `EXECUTE`. Local PostgreSQL 16 source evidence covers 16 migration/tamper cases, exact function restoration, same-name `CHECK(true)`, owner-forged high history rejection, direct runtime rollback/insert denial, NOINHERIT `SET ROLE` detection and actual migration-runner rejection, alternate-schema reset resistance, first-acceptance contention, and stale rejection. Authorization expiry is checked at the chain-bound finalized activation timestamp, while checkpoint freshness remains wall-clock bounded. This is source/local rehearsal evidence, not proof that Render roles, schema pin, or migration 002 are live. | Preprovision the pinned Render schema and distinct owner/runtime roles, inspect the role graph, apply the reviewed migration, retain redacted OID/function/ACL/privilege evidence and production rehearsal JSON, remove the owner credential from the web child, then verify live source and `target: null` before any later activation |

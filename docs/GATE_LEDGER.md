...
| No unified production funding authorization | `schemas/production-launch-authorization.schema.json` and `p42-prizes production-launch-authorization-validate` compose exact-byte legal, governance, incident, adversarial, operational, release-verification, completed-deployment, explorer, and ten signed independent math-review packets into one expiring release-bound digest. `p42-funding-activation-plan` emits an exact-ten, runtime-code-pinned plan with global authorize/arm/open barriers. `p42-funding-activate` now executes at most one transaction per run, reconstructs every barrier from dual-RPC common-finalized state, reruns validation before each new signature, journals raw bytes before broadcast, and advances only on finalized agreement. Contracts bind authorization expiry on-chain. This remains source evidence only: no current-deployment rehearsal or real production packet exists, independent signer custody is not approved, and the retained reconciliation/indexer checkpoint plus portal donation publication do not yet consume one completed activation digest. Until every consumer agrees, this gate remains open and no real-ETH funding target is authorized | Engineering + security + counsel + math reviewers |

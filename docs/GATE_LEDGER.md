...
| GitHub Actions workflow | Live source and deploy pass; production receipt blocked on v3 ratification | `.github/workflows/ci.yml` is published. [`main` run 29385762495](https://github.com/techno-optimist/p42-prizes/actions/runs/29385762495) passed all six lanes for exact source head `f81bb45406957109717d3cfbbc8b44158bc44f73`, including Ubuntu 22.04 and 24.04 SP1 rebuild/replay jobs. The authenticated Render record binds live deploy `dep-d9bfisl7vvec73a5p2ag` to that same deploy-relevant commit, and the release guard passed the committed route/configuration/projection policy. The prior [`source-release receipt`](evidence/source-release-current.json) remains valid historical evidence for `f6dba23d8d76d4ff833077242397ef71727368db` but is not current after direct-pushed deploy commits `c20f1ec3e53c7bbfdb562723d0c995fbe2ae3fc0` and `f81bb45406957109717d3cfbbc8b44158bc44f73`. This evidence-only PR records the gap; it does not retroactively establish review provenance for those commits | Establish an externally pinned source-release v3 policy, independently review and threshold-sign a bootstrap ratification binding the exact `c04c37b..f81bb45` deploy patch and final tree, then publish a chained v3 receipt with pinned workflow identity and fresh live probes. Retain a completed push run and release-guard record for every source release. GitHub returned `403` for branch protection/rulesets and `404` for private vulnerability reporting; those are explicit repo-owner/account blockers |

...
| Portal state/rate limits/idempotency/events | Live state authority passed; checkpoint owner-function upgrade pending production provisioning | Historical live-state evidence remains `docs/evidence/portal-db-cutover-2026-07-13.json`. Current source confines checkpoint changes to a pinned migration-owner `SECURITY DEFINER` transition function and serves an exact unchanged checkpoint through a separately pinned, read-only `FOR SHARE` function using indexed maxima; a miss rolls back before fresh authority attestation and serialized transition. Runtime receives only `SELECT` plus those exact function `EXECUTE` grants, with direct DML, `TRUNCATE`, dangerous `SET ROLE`, external triggers, and authority drift rejected. Local PostgreSQL 16 evidence covers 18 migration/tamper cases, including contiguous block/time regression and nonadjacent full-identity replay, plus stale-transition contention and a 10,000-acceptance/six-reader rehearsal that observes all six reads in flight, proves zero mutual reader blockers, requires the serialized transition to be blocked only by held-reader PIDs, and enforces a 2,000 ms post-barrier completion ceiling. Authorization expiry is checked at the chain-bound finalized activation timestamp, while checkpoint freshness remains wall-clock bounded. This is source/local rehearsal evidence, not proof that Render roles, schema pin, or migration 002 are live. Migration 002 is new relative to base `e083f94`; unpublished intermediate draft schemas were never pushed or deployed and are not production compatibility inputs. | Preprovision the pinned Render schema and distinct owner/runtime roles, inspect the role graph, apply the reviewed migration, retain redacted OID/function/ACL/privilege evidence and production rehearsal JSON, remove the owner credential from the web child, then verify live source and `target: null` before any later activation |

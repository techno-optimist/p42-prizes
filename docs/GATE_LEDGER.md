...
July 21 snapshot: `main` is
`96cc266c309aedaea6e8c8d34068990dc722c515`. Exact-main push run
[`29855539407`](https://github.com/techno-optimist/p42-prizes/actions/runs/29855539407)
passed the ordered seven-job gate, including both Ubuntu objective-program
producers and the cross-image reproducibility aggregator. Explicit Render deploy
`dep-d9fstifaqgkc738nl4n0` now serves that exact commit. Direct and public
health both return the exact
...
| GitHub Actions workflow | Exact `96cc266` main and the 57-observation live guard pass; production v3 receipt remains open | `.github/workflows/ci.yml` is published. Exact-main push run [`29855539407`](https://github.com/techno-optimist/p42-prizes/actions/runs/29855539407) passed the ordered seven-job gate for `96cc266c309aedaea6e8c8d34068990dc722c515`. Explicit deploy `dep-d9fstifaqgkc738nl4n0` serves that exact commit. Direct/public health is database-ready, all ten exact v3 funding responses remain `target: null`, and the guard retained 32/32 authority plus 25/25 supplemental checks. This is current unfunded-portal evidence, not testnet settlement, real-ETH authorization, or an externally ratified v3 source-release receipt. | Provision the externally pinned source-release v3 policy/trust root, independently review and threshold-sign every required migration interval, and publish the chained v3 receipt. Private branch protection/rulesets and private vulnerability reporting remain repo-owner/account-tier blockers. |
...
| Portal API | Live unfunded routes use the rehearsed PostgreSQL 18.4 cutover, schema validation, raw-byte reveal, strict hash-only mutation credentials, fail-closed transactions, idempotency, and atomic database rate limits. Challenges remain honestly unimplemented | Retain cutover/reconciliation evidence; provision scoped production credentials; add abuse monitoring/payload quarantine; review auth/session policy |
...
| Verifier execution | Seventeen exact problem packages exist and the frozen ten-board source cohort is binding-tested. The portal invokes package verifiers on raw bytes with a wall-clock timeout; chain-linked jobs require the fail-closed Docker sandbox, immutable image references, and aggregate cgroup memory/PID controls. `admit-host`/`admit-matrix` enforce typed N-host evidence locally; `admit-ready` rejects placeholder images before funding | Defensible final cohort, pullable production image digests, production Linux/DGX sandbox rehearsal, and collected N-host identical-verdict matrices |

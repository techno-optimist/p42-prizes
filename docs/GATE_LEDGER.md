...
July 16 snapshot: the source-release v3 enforcement implementation and the
combined production-authority hardening are merged. The external v3 trust root,
bootstrap ratification, and current receipt remain open. Render serves exact source commit
`5a4d25d3862d75d2b573e93f33f60ad72662aef7`; the authenticated release guard
binds that commit to the canonical board projection and reports 12/12 healthy
routes. The public funding-target API remains fail-closed with no published
target, and Build Week is absent from site navigation while its direct
historical route remains available. This is source and live-portal evidence,
not funding activation. The current source still lacks a fresh canonical
...
| --- | --- | --- | --- |
| Gate 0: Public repo / local pilot | Mostly green; live source and deploy checks pass, but the production receipt is blocked on v3 ratification and two repository-account controls remain | Local verifier, web/API tests, fail-closed challenge/onramp, live fail-closed PostgreSQL authority, security policy text, published `.github/workflows/ci.yml`, and a schema-validated historical v2 receipt. The exact current evidence and authority gap are recorded in the GitHub Actions row below; the historical receipt is not current-release authorization. | Establish the externally pinned and threshold-ratified v3 source-release chain; retain a successful push run and release-guard record for each newly merged source release; upgrade the private repository/account tier or make the repository public so protected branch/ruleset enforcement is available; repo owner enables private vulnerability reporting through a supported GitHub surface |

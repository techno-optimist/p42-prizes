...
- [x] Source-level operator and resolver runtime templates use distinct unprivileged accounts, a bounded `--once` supervisor with finite failure budgets, timeout escalation, private atomic self-hashed journals, an account-separated failure sink, explicit operator sandbox staging, systemd `LoadCredential` key custody, and `LimitCORE=0`. Their complete `ExecStart` commands share the production CLI contract enforced by the executables, including immutable operator provisioning, resolver quorum paths, explicit Arweave publication, and two retrieval endpoints; legacy receipt/endpoints environment variables fail closed when they conflict. The CI-called systemd aggregate checks both units, exact entrypoint preflight tests exercise the staging/publication paths, and Python source tests verify the worker binds staged payloads plus per-container Docker memory/PID/OOM/core controls. Systemd limits cover the service tree only; Docker daemon cgroup placement and live per-container enforcement remain deployment evidence, along with native installed-unit verification, live independent RPCs, funded durable publication, signer provisioning, queue latency under load, and an on-chain poll.

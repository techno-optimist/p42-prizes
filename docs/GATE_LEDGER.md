...
| Portal state/rate limits/idempotency/events | Live state authority passed; checkpoint epoch upgrade pending production provisioning | Historical live-state evidence remains `docs/evidence/portal-db-cutover-2026-07-13.json`. Current source adds a transaction-locked control row plus append-only identity epochs and checkpoint acceptances in `web/migrations/002_indexer_checkpoint_high_water.sql`; `web/src/lib/indexer-high-water.ts` rejects monotonicity/history/privilege/trigger/readback failures before funding publication. Local PostgreSQL 16 evidence covers exact-schema tampering including same-name `CHECK(true)`, separate owner/runtime roles, first-acceptance contention, and stale rejection. This is source/local rehearsal evidence, not proof that Render roles or migration 002 are live. | Provision distinct Render owner/runtime roles, apply the reviewed migration, save redacted catalog/privilege evidence and production rehearsal JSON, remove the owner credential from the web process, then verify live source and `target: null` before any later activation |
...
- Activated funding publication additionally requires the append-only checkpoint epoch gate. Runtime may insert history and update only the locked control pointer; table ownership, schema creation, trigger authority, non-internal triggers, historical identity replay, or persisted-value mismatch suppresses every funding target.

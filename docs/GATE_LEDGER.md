...
| Canonical 46-contract topology not materialized | `protocol/canonical-topology-v1.json` is the single ordered authority for six shared contracts plus four contracts on each of ten boards. The current ceremony still plans the historical 43 identities, so production preflight intentionally rejects it before pending-nonce lookup, address reservation, signed-journal creation, or broadcast. Existing 43-address manifest/explorer/role/authorization validators remain historical consumers; the funding-activation planner independently requires 46 unique canonical addresses and rejects their output before any operation plan is produced. Remaining: implement factory CREATE2 deployment/provenance for all twenty managers, deploy and freeze the resolver quorum against the exact ten challenge managers, then migrate manifest, schemas, explorer, role acceptance, indexer, reconciliation, and launch authorization together | Engineering + hostile reviewer |

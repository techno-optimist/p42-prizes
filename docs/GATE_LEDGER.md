...
- [x] Source-level operator and resolver runtime templates use distinct unprivileged accounts, a bounded `--once` supervisor with finite failure budgets, timeout escalation, private atomic self-hashed journals, an account-separated failure sink, explicit operator sandbox staging, systemd `LoadCredential` custody for signing and RPC URLs, and `LimitCORE=0`. Their complete `ExecStart` commands share the production CLI contract enforced by the executables, with no RPC URL or rootful Docker socket in process arguments; legacy receipt/endpoints/RPC environment fallbacks fail closed. The operator requires the reviewed same-user rootless Docker daemon, subordinate-ID/userns prerequisites, a private socket, and a pre-poll socket check. The CI-called systemd aggregate checks both runtime units plus the rootless daemon/config, exact expanded-unit fixture tests exercise credential/staging/socket/RPC publication preflight, and Python source tests verify the worker binds staged payloads, passes the rootless endpoint to Docker, refuses pre-lease outages, and preserves per-container memory/PID/OOM/core controls. This is source/static and credential-free fixture evidence only; installed Linux/DGX rootless-daemon operation, live Docker cgroups/jobs, native installed-unit verification, live independent RPCs, funded durable publication, signer provisioning, queue latency under load, and an on-chain poll remain deployment evidence.

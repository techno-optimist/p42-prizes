...
| No Base Sepolia deployment | `contracts/scripts/deploy-base-sepolia.js` and `deployments/base-sepolia/p42-prizes.example.json` now define the artifact, and the full deploy → manifest → reconcile pipeline is dry-run-verified end-to-end against the post-audit contracts on a local chain (5 contracts deployed + wired, problem registered, reconciliation `ok=true`, 7/7 invariant checks); still need the real credentialed deploy txs, committed `p42-prizes.json`, verified source links, addresses, constructor metadata, and role assignments | Deployer credential owner |
...
| No funding reconciliation | `contracts/scripts/reconcile-base-sepolia.js` now defines a read-only report over deposits, credits, claims, submissions, challenges, and pool/ledger balances (a field bug reading a nonexistent `Challenge.resolverBondWei` is fixed — resolver bonds are read from the `resolverBonds` mapping — and the report validates in the dry-run); still need a real committed Base Sepolia report, reorg policy, monitoring, portal read integration, and signed ops review | Engineering + ops |

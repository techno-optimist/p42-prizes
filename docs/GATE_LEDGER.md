...
| GitHub Actions workflow | Live source and deploy pass; production receipt blocked on v3 ratification | `.github/workflows/ci.yml` is published. [`main` run 29400163976](https://github.com/techno-optimist/p42-prizes/actions/runs/29400163976) passed all seven required jobs for exact source head `869839daa047305efde7bf6850a2cfef4abac87a`, including Ubuntu 22.04 and 24.04 SP1 rebuild/replay producers and their cross-image `SP1 objective-program reproducibility` comparator. The authenticated Render guard subsequently bound live commit `869839daa047305efde7bf6850a2cfef4abac87a`, runtime commit `e57327efbad1fce38578e667ebeb974f6905b3e2`, the canonical board projection, and 12/12 healthy routes. The prior [`source-release receipt`](evidence/source-release-current.json) remains valid historical v2 evidence for `f6dba23d8d76d4ff833077242397ef71727368db`, not current authorization. V3 source validation requires the exact ordered seven-job set, including the cross-image comparator, plus PR or threshold-bootstrap authorization for every non-exempt first-parent source commit; it recursively replays predecessor provenance before accepting its baseline, authenticates historical and current PR rows, keeps Render relevance separate, and rejects approvals submitted after merge. Six producer jobs alone cannot authorize a v3 receipt | Establish the externally pinned source-release v3 policy, independently review and threshold-sign every required migration interval, then publish a chained v3 receipt with pinned workflow identity and fresh live probes. Retain a completed push run and release-guard record for every source release. GitHub returned `403` for branch protection/rulesets and `404` for private vulnerability reporting; those are explicit repo-owner/account blockers |

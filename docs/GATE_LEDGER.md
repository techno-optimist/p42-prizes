...
| Gate 1: Base Sepolia testnet | Blocked | Python reference model, portal-local commit/reveal, local Hardhat contract scaffold tests for registry/pool/payout/submission/challenge/resolver invariants, and Base Sepolia deployment-manifest scaffold | Deployed verified contracts, testnet addresses, real DA/permanence verification, integrated resolver transcript, indexer reconciliation, adversarial run report |

...
- [x] `protocol/production-board-bindings-v1.json` binds the ordered exact-ten cohort to current problem/spec/schema/seed bytes, installation-independent verifier source trees, objective classifications, and exact seed reports. `make verify-production-board-bindings` replays all ten. This is source integrity only: nine proof guests are missing, Hadamard is mock-only, provenance is incomplete, and all ten independent math reviews remain pending.

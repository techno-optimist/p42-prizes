...
| --- | --- | --- | --- |
| Gate 0: Public repo / local pilot | Mostly green; latest merged source-release CI and live release guard pass, two repository-account controls remain | Local verifier, web/API tests, fail-closed challenge/onramp, live fail-closed PostgreSQL authority, security policy text, published `.github/workflows/ci.yml`, and [`source-release evidence`](evidence/source-release-2026-07-14-8cfbd36.json) binding green post-merge [`main` run 29326212579](https://github.com/techno-optimist/p42-prizes/actions/runs/29326212579) plus live Render deploy `dep-d9b14cbbc2fs73a3n3h0` to source release `8cfbd366ec206e244aa125535cfca7b7adca4fbb` | Retain a successful push run and release-guard record for each newly merged source release; upgrade the private repository/account tier or make the repository public so protected branch/ruleset enforcement is available; repo owner enables private vulnerability reporting through a supported GitHub surface |
...
| GitHub Actions workflow | Pass for workflow presence, latest merged source-release run, and live deploy binding | `.github/workflows/ci.yml` is published. Post-merge [`main` run 29326212579](https://github.com/techno-optimist/p42-prizes/actions/runs/29326212579) passed all six lanes for exact source release `8cfbd366ec206e244aa125535cfca7b7adca4fbb`, including independent Ubuntu 22.04 and 24.04 SP1 rebuild/replay jobs. The authenticated Render record binds live deploy `dep-d9b14cbbc2fs73a3n3h0` to that commit, and `make verify-render-release` passed all 8 route/configuration/projection checks. The checked-in [`source-release evidence`](evidence/source-release-2026-07-14-8cfbd36.json) records both authorities and the fail-closed portal-projection posture | Retain a completed push run and release-guard record for every newly merged source release. GitHub returned `403` for branch protection/rulesets and `404` for private vulnerability reporting; those are explicit repo-owner/account blockers |
...
- [x] Required-check CI and the release guard pass for exact merged source release `8cfbd366ec206e244aa125535cfca7b7adca4fbb` ([run 29326212579](https://github.com/techno-optimist/p42-prizes/actions/runs/29326212579), [`release evidence`](evidence/source-release-2026-07-14-8cfbd36.json)); each later source merge must earn its own successful push run and live binding before release.

...
| Canonical 47-contract topology not deployed | `protocol/canonical-topology-v1.json` is the single ordered authority for seven shared contracts plus four contracts on each of ten boards. The ceremony now materializes all 47 before signing: direct roots including the capsule-attested objective-verifier gateway, pool/ledger contracts, twenty factory CREATE2 manager children, and a resolver quorum frozen against the exact ten challenge managers. Manifest, explorer, role-acceptance, indexer, reconciliation, and launch-authorization consumers require the same ordered topology. Factory children carry transaction, event, CREATE2, and historical configuration-getter evidence. Remaining: complete hostile review, merge the atomic migration, run the Base Sepolia ceremony, and independently verify the resulting dossier | Engineering + hostile reviewer |
...
- [ ] The source-level permissionless objective-proof path is structurally wired and locally tests package/program/gateway/runtime bindings plus both settlement outcomes. The first P42-owned SP1 v6.1 program now re-runs the exact `hadamard-668-defect` predicate, reconstructs the complete Solidity reveal/challenge/package/journal hash chain, and emits exactly 32 public bytes; its path-remapped Linux x86 build is `sha256:2cda9c4c278b5c5b72b56417d6c0d2c7a15f045898e69ead9b75a61026ca13a6` with vkey `0x0016d27da623507d3e323cc70a9da608e55879c4d74481566040fc38bf7f9799`. GitHub Actions retained that exact binary and an ARM host independently derived the same vkey and replayed the fixture in 53,222,072 instructions, producing journal `0xb7fc3d85ad6a8606edb944c1883fd8feed8363a74b84697d81439f3a69c664fc`. This is cross-host execution evidence, not cross-host source-build reproducibility, a proof, or activation: the ARM source build has a different ELF and vkey. The production gateway remains immutably inactive and launch authorization v1 permanently rejects activation. A second matching Linux source build, a genuine Groth16 vector, worst-case malformed-input benchmarks and economic sizing, the other nine board programs, audit, a new active gateway/release/authorization version, deployment, and adversarial testnet rehearsal remain required before this gate can close or funding can be authorized.
...
- Governance setup continuation now has a release/config/timelock-bound durable operation journal. Production continuation remains observation-only, consumes the dual-RPC finalized anchor, records primary or deterministic override execution evidence, and reuses the fenced dead-owner-reclaiming journal lock. The 47-contract/110-operation local rehearsal recovers a mined-before-journaled execution and still proves every funding gate false. This is source/rehearsal evidence, not a deployed Gate 1 pass.

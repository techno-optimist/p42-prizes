...
| No unified production funding authorization | `schemas/production-launch-authorization.schema.json` and `p42-prizes production-launch-authorization-validate` now compose exact-byte legal, governance, incident, adversarial, operational, release-verification, completed-deployment, explorer, and ten signed independent math-review packets into one expiring release-bound digest. The validator refuses test trust, mismatched release/deployment/verifier pins, stale explorer evidence, noncanonical board identities, and copied artifact hashes. This is source evidence only: no real production packet exists, and deployment completion, reconciliation/indexing, and portal donation publication do not yet consume the digest. Until all three consumers fail closed on one validated digest, this gate remains open and no real-ETH funding target is authorized | Engineering + security + counsel + math reviewers |

...
| SP1 Fiat-Shamir dependency advisory | **Activation blocker.** All four objective-program lockfiles resolve SP1 6.1.0 through `slop-challenger` to `p3-challenger 0.3.2-succinct`, affected by high-severity `GHSA-vj64-rjf3-w3v7`. The advisory concerns Fiat-Shamir transcript malleability and entropy loss. SP1 6.2.2 through the current 6.3.1 release line are not accepted as remediation: they pin prerelease `0.4.3-succinct`, which GitHub still classifies below patched stable `0.4.3`, and its published `multi_field_challenger.rs` remains byte-identical to the affected implementation. The stable patched crate changes proof-system APIs and cannot safely be forced beneath SP1. The low-severity `p3-symmetric` variable-length-hash advisory has no patched release in the affected line, and `lru` is not patched until 0.16.3. `make objective-dependency-security-gate` now scans every objective lockfile, verifies exact SP1 Git pins, and fails on all three published ranges. See `docs/SP1_DEPENDENCY_SECURITY_2026_07_18.md`. No frozen identity changed. | Keep objective proofs and funding activation disabled. Obtain or sponsor a Succinct release containing the actual upstream challenger fix, then cut a new objective artifact version and rerun semantic differential vectors, hosted x86 dual-image builds before any byte-reproducibility claim, ELF/vkey derivation, genuine proof verification, independent reproduction, external audit, and release/gateway authorization. Keep the alerts open until that replacement ceremony passes. |
...
| Immutable image registry not populated | `docs/VERIFIER_IMAGE_REGISTRY.md`, `scripts/release_verifier_images.py`, and `schemas/verifier-image-release.schema.json` define a fail-closed all-ten `S/R` publication ceremony. It builds both required platforms from a private read-only `git archive` of exact `S`, binds both commit archives and source hashes, walks the raw OCI index/manifest/config digest chain, and journals every push before producing a self-hashed non-overwriting v2 dossier at `R`. Canonical contract preparation, offline verification, and production deployment reject v1 and invoke `admit-release-ready` with independently pinned dossier and journal bytes from exact `R`. Each admission host's atomic v3 bundle contains and independently indexes all 20 raw rehearsals plus ten signed summaries. The non-publishing plan at `deployments/dgx-image-plan/2026-07-12/` deterministically resolves all ten source hashes and both target platforms; a separate unsigned DGX operator observation records the checkout and tool environment and found no configured registry auth. Neither file is a host attestation. No image was built or pushed. Still need narrowly scoped registry write credentials, provisioned immutable retention/access policy, real reviewed index/platform digests, and durable independent-host matrix artifacts for every fundable board | Engineering + registry owner |

...
| Immutable image registry not populated | `docs/VERIFIER_IMAGE_REGISTRY.md` defines the registry fields and `p42-prizes admit-ready` rejects placeholder images or matrix/manifest mismatches. Source bundle v2 binds every canonical build input, rejects links/special files, validates OCI config, forces the admitted Python entrypoint, and uses bounded archive inspection; each signed host record binds its resolved platform image ID without creating a source/image fixed-point. The ceremony runs that gate before any deployment reservation and binds the validated matrix digest to its registry hash. Still need real reviewed registry image/index digests, platform images, and durable matrix artifacts for every board | Engineering |

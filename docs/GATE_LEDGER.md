...
July 18 snapshot: integration source commit
`1b65b84b13dbe350339bcd985b44b5224e6c6df7` is merged to `main` and deployed
on Render. The authenticated release guard binds branch, runtime, and live
commit to that exact SHA, binds board projection
`sha256:ec42d67bc8d67d3732fc363b371b6c2e4acffa256c107df887ad3501cb6a0137`,
reports 32/32 healthy routes, and requires all ten funding targets to be null.
Exact-main CI run
[`29645758684`](https://github.com/techno-optimist/p42-prizes/actions/runs/29645758684)
has four ordinary lanes green while its two SP1 producers and cross-image
comparator remain unfinished. The external v3 trust root, bootstrap
ratification, and current receipt remain open, so this is public-unfunded portal
evidence rather than current release authorization or funding activation.
Build Week is absent from site navigation while its direct historical route
remains available. Multi-platform GHCR index identities have been published for
all ten boards, but their draft dossier/reinspection evidence is not merged or
independently approved, layer-blob availability is not claimed, checked-in
problem bindings still contain placeholder image identities, and no trusted
four-host matrix has admitted them. The current source therefore still lacks
admitted immutable verifier images, a fresh canonical 47-contract deployment,
current manifest/reconciliation, live agent wallet run, strict open-witness
launch boards, external audit, and counsel approval. Historical Base Sepolia
evidence for old bytecode cannot close a current deployment gate.
...
| Gate 2: Real ETH pilot | Blocked | Conservative copy, gate docs, tested admission tooling, immutable-image `admit-ready` scaffold, signed exact-ten host collector, draft wallet/session policy, opt-in mutation API-key gate, and legal memo validator | External audit, counsel-signed legal memo, KYC/sanctions/ToS approval, admitted immutable registry image digests with independently reviewed evidence, collected trusted four-host verifier matrix, named multisig/guardian, distributed state/abuse controls, incident drill, bug bounty |
...
| GitHub Actions workflow | Current exact-head run in progress; live release guard passed; production receipt blocked on external v3 ratification | `.github/workflows/ci.yml` is published. Current [`main` run 29645758684](https://github.com/techno-optimist/p42-prizes/actions/runs/29645758684) targets exact merged source head `1b65b84b13dbe350339bcd985b44b5224e6c6df7`: Python verifier, autonomous agent, portal, and contract jobs pass; both Ubuntu SP1 producers and their dependent cross-image comparator must still pass. The authenticated Render guard independently binds branch head, runtime commit, and live commit to that same SHA, binds board projection `sha256:ec42d67bc8d67d3732fc363b371b6c2e4acffa256c107df887ad3501cb6a0137`, and reports 32/32 healthy routes with all funding targets null. The prior [`source-release receipt`](evidence/source-release-current.json) remains valid historical v2 evidence for `f6dba23d8d76d4ff833077242397ef71727368db`, not current authorization. V3 source validation requires the exact ordered seven-job set plus PR or threshold-bootstrap authorization for every non-exempt first-parent source commit; it recursively replays predecessor provenance, authenticates historical/current PR rows, keeps Render relevance separate, and rejects approvals submitted at or after merge. Six producer jobs alone cannot authorize a v3 receipt | Complete the exact-head seven-job CI and independently replay the exact ten retained artifacts. Provision the externally pinned source-release v3 policy/trust root, independently review and threshold-sign every required migration interval, and publish the chained v3 receipt with pinned workflow identity and fresh live probes. Retain a completed push run and release-guard record for every source release. A July 16 recheck still returns GitHub `403` for branch protection/rulesets and `404` for private vulnerability reporting; those remain explicit repo-owner/account blockers |
...
| SP1 Fiat-Shamir dependency advisory | **Activation blocker.** All four objective-program lockfiles resolve SP1 6.1.0 through `slop-challenger` to `p3-challenger 0.3.2-succinct`, affected by high-severity `GHSA-vj64-rjf3-w3v7`. The advisory concerns Fiat-Shamir transcript malleability and entropy loss. SP1 6.2.2 is not accepted as remediation: its published `multi_field_challenger.rs` remains byte-identical to the affected implementation even though the dependency label changed. The low-severity `p3-symmetric` variable-length-hash advisory has no patched release and is tolerable only while proof activation is disabled and protocol input lengths remain fixed; the `lru` advisory's affected mutable iterator API is not reachable in the pinned SP1 call graph. Any dependency update changes the sealed source/lockfile identity and may change ELF/vkey identities. | Keep objective proofs and funding activation disabled. Obtain or sponsor a Succinct release containing the actual upstream challenger fix, then cut a new objective artifact version and rerun semantic differential vectors, dual-image builds, ELF/vkey derivation, genuine proof verification, independent reproduction, external audit, and release/gateway authorization. Keep the high-severity alerts open until that replacement ceremony passes. |

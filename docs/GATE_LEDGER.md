...
- Funding publication is fail-closed at the legal boundary. General problem list/detail routes redact pool and donation destinations and do not carry a `donationTarget`; only the dynamic same-origin funding endpoint may eventually provide transfer instructions. Phase 0 preserves the exact `funding-target/v3` envelope with `target: null`. Expanded active targets have a separate `funding-target/v4` schema containing release-artifact references, but current `p42-production-launch-authorization/v1` does not sign those references. Raw environment assertions therefore cannot activate v4. A future authorization version must cryptographically bind the exact legal artifact bytes/references into the signed authorization and the same authorization bytes/digest into activation completion and fresh reconciliation before the endpoint may emit v4. The v4 client parser hides the full address and BaseScan action until explicit artifact and policy acknowledgement, resets acknowledgement on binding or focus/resume revalidation, and retains the second trusted click before wallet launch. This is source evidence only; no current artifact authorizes funding.

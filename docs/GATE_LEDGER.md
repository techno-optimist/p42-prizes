...
July 17 snapshot (2026-07-17 05:36 MDT): the source-release v3 enforcement and
combined production-authority hardening are integrated at PR #140 candidate head
`10f48e8c7951482b0f28dee9c2bdc34320ba6719`; they are not merged to `main`.
PR #140 run
[`29576211886`](https://github.com/techno-optimist/p42-prizes/actions/runs/29576211886)
currently has portal, autonomous-agent, Python, and contracts jobs green, with
both SP1 producer jobs pending. That is a time-stamped current snapshot, not a
prediction or completed exact-head pass. PR #141 is evidence-scoped and does not
establish source merge, deployment, or live authority. The external v3 trust
root, bootstrap ratification, and current receipt remain open. Historical live
evidence for exact source commit
`5a4d25d3862d75d2b573e93f33f60ad72662aef7` bound that commit to the canonical
board projection and reported 12/12 healthy routes; the live portal still serves
that commit. The PR #140 candidate has no corresponding live release-guard
evidence. Its hardened portal checkpoint authority is source/local evidence only;
the legacy live database passed its historical check, but production cutover is
pending. Funding activation has not occurred: live targets remain `null`, and
candidate behavior remains local-only. Build Week is absent from site navigation
while its direct historical route remains available. The current source still
lacks a fresh canonical
...
| --- | --- | --- | --- |
| Gate 0: Public repo / local pilot | Mostly green; PR #140 is an unmerged integration candidate, the last live source/deploy check is historical, the candidate is not deployed, the production receipt is blocked on v3 ratification, and repository-account controls remain | Local verifier, web/API tests, fail-closed challenge/onramp, a historical pass against the legacy live PostgreSQL database, source/local hardened-checkpoint evidence, security policy text, published `.github/workflows/ci.yml`, and a schema-validated historical v2 receipt. The exact historical evidence and current authority gap are recorded in the GitHub Actions row below; none authorizes the candidate. | Complete PR #140 exact-head CI, merge it to `main`, deploy it, and retain a fresh release guard; establish the externally pinned and threshold-ratified v3 source-release chain; retain a successful push run and release-guard record for each newly merged source release; upgrade the private repository/account tier or make the repository public so protected branch/ruleset enforcement is available; private vulnerability reporting is unavailable on the current private plan |
...
| Portal state/rate limits/idempotency/events | Historical legacy database passed; hardened checkpoint authority is source/local only; production cutover pending | Historical live legacy-database evidence remains `docs/evidence/portal-db-cutover-2026-07-13.json`; it does not prove the hardened authority is live. The PR #140 candidate confines checkpoint changes to a pinned migration-owner `SECURITY DEFINER` transition function and serves an exact unchanged checkpoint through a separately pinned, read-only `FOR SHARE` function using indexed maxima. Distinct migration-owner and runtime identities are the target production design, not a description of current live provisioning. Local PostgreSQL 16 evidence covers 18 migration/tamper cases and the 10,000-acceptance/six-reader rehearsal. The starting live database has legacy public tables and no `p42_portal` schema. PR #142 is a draft/source provisioning candidate; it is not merged, and transfer repair must not be claimed merged. | Merge the reviewed source, then provision the pinned Render schema and distinct owner/runtime roles; perform the exact transfer ceremony, which is not an ordinary runtime import; inspect the role graph; apply the reviewed migration; retain redacted OID/function/ACL/privilege and production rehearsal evidence; remove the owner credential from the web child; then verify the deployed source and live `target: null` before any later activation |
| Funding activation | Not activated | The live portal remains at `5a4d25d3862d75d2b573e93f33f60ad72662aef7` with funding targets `null`. PR #140 funding behavior is candidate source/local evidence only; no production activation packet, canonical Base Sepolia deployment, or live activation checkpoint exists. | Keep targets `null` until the reviewed source is merged and deployed and every deployment, authority, governance, reconciliation, and activation artifact passes |
| Security disclosure text | Pass in repo | `SECURITY.md` | Private vulnerability reporting is unavailable on the current private plan; the repo owner must upgrade the plan or make the repository public before enabling and testing it |
| GitHub Actions workflow | Historical source/deploy pass; PR #140 exact-head CI in progress; `main`, deployment, and live release guard pending; production receipt blocked on external v3 ratification | `.github/workflows/ci.yml` is published. Historical [`main` run 29533825184](https://github.com/techno-optimist/p42-prizes/actions/runs/29533825184) passed all seven required jobs for exact merged source head `5a4d25d3862d75d2b573e93f33f60ad72662aef7`, and the authenticated Render guard bound the live portal to that SHA with 12/12 healthy routes. At 2026-07-17 05:36 MDT, PR #140 candidate-head run [`29576211886`](https://github.com/techno-optimist/p42-prizes/actions/runs/29576211886) has portal, autonomous-agent, Python, and contracts green while both SP1 producers remain pending; no success is predicted. PR #141 is evidence-scoped and does not establish source merge, exact-head completion, deployment, or live authority. The candidate is not on `main` and is not covered by a live guard. The prior [`source-release receipt`](evidence/source-release-current.json) remains historical v2 evidence, not current authorization. | Wait for the exact PR #140 run to reach a terminal state; require the complete seven-job set, then merge to `main`, deploy, and retain a fresh release guard. Provision the externally pinned source-release v3 policy/trust root, independently review and threshold-sign every required migration interval, and publish the chained v3 receipt with pinned workflow identity and fresh live probes. Protected releases remain unavailable on the current private plan, as does private vulnerability reporting |
...
- Funding target publication is deadline-aware and fail-closed. The dynamic same-origin `no-store` endpoint publishes only the target authorized by a fresh activation checkpoint; the page and React Server Component payload contain no target bytes at or after the funding deadline. Client-side binding, retry, clipboard, and boundary races have regression coverage. The live July 16 release intentionally returns `target: null` because no production activation packet exists.

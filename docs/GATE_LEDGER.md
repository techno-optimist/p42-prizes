...
| No unified production funding authorization | `schemas/production-launch-authorization.schema.json` and `p42-prizes production-launch-authorization-validate` compose exact-byte legal, governance, incident, adversarial, operational, release-verification, completed-deployment, explorer, and ten signed independent math-review packets into one expiring release-bound digest. `p42-funding-activation-plan` now invokes that validator directly and emits an exact-ten, runtime-code-pinned plan with global authorize/arm/open barriers. Contracts bind authorization expiry on-chain, so delayed timelock arm or pool-open execution fails after expiry. This remains source evidence only: the durable multi-signer signing/broadcast state machine is incomplete, no real production packet exists, and reconciliation/indexing plus portal donation publication do not yet consume the digest. Until all consumers fail closed on one validated digest, this gate remains open and no real-ETH funding target is authorized | Engineering + security + counsel + math reviewers |

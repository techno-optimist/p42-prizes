...
| Solver winnings reinvestment | Source-complete, deployment pending | A solver may atomically direct a matured award into another active pool in the same frozen canonical registry. The source claim rolls back on any destination failure, the destination sponsorship remains attributed to the solver, and frontier/title records are unchanged. The agent accepts only destination pools in its validated deployment manifest; the contract independently checks registry identity, problem binding, explicit freeze, and active funding before consuming the claim | Deploy, externally audit, reconcile, and expose only authorization-bound production destinations |
...
| --- | --- | --- |
| Contract system incomplete | Local Hardhat 3 source and tests now cover the core registry, escrow, payout, submission, challenge, resolver, bounded permissionless full-pause recovery, sponsor-refund, fee, forced-ETH, and restricted-rollover paths. The deterministic differential campaign in `contracts/test/p42-economic-state-machine.test.js` executes 100 unique seeds with 15,000 randomized action attempts, checks model/chain agreement and a separately chain-derived liability/conservation oracle after every action, requires at least 5,000 successful transitions plus explicit pause-claim, duplicate-withdrawal, sponsor-refund, rollover, and forced-ETH coverage witnesses, and emits the replayable seed/action trace on failure. This is local source evidence only, not a completed audit; it still needs formal review, fresh deployment, external audit, deployment-specific pause-recovery rehearsal, and counsel/product approval of the implemented close/refund/rollover policy | Engineering + external auditor |
...
| No current funding/indexer reconciliation | The committed reconciliation is historical evidence against old bytecode only. Production completion/publication now requires an immutable release-bound Base Sepolia finality policy, agreement between two operator-distinct RPCs on canonical finalized/safe L2 plus OP Stack L1-origin/finality evidence, and an immediate monotonic/reorg recheck; fixtures remain explicit and non-actionable. Current source still needs a deployed runtime, chunked log reads where RPCs cap `eth_getLogs`, complete lifecycle/recovery-event replay, running indexer, real funding deposits, monitoring, portal read integration, and signed ops review | Engineering + ops |
| No live reveal watcher or agent wallet run | `docs/VERIFIER_RUNNER.md` defines DGX CHRONOS/Hermes as the immediate verification worker and keeps runner output outside the trust root. `p42-prizes runner-burst-validate` now requires hash-bound files beneath a secure artifact root, derives queue/transcript/alert/guard claims, and leaves unsigned reports explicitly non-attesting; a trusted runner signature is required for `gate_passed`. `agent/operator.mjs` has the corresponding finalized-log, pinned-sandbox, exact-policy, raw-transaction, and reorg-reconciliation runtime. A non-value-moving DGX runner-health rehearsal is recorded at `deployments/dgx-runner-rehearsal/2026-07-11/`: it proved signed fail-closed and green producer states, exposed and closed a Python/Node byte-limit mismatch, and obtained `green_v2` from the corrected consumer. It used a sentinel contract and is not deployed evidence. Still need one deployed event-to-sandbox-to-transcript-to-challenge rehearsal, durable transcript publication, a real signed burst-drill packet, bounded challenge-key policy rehearsal, and a current live agent wallet/operator run | Engineering + ops |
| Resolver transcript path not deployed/integrated | Local `P42ChallengeManager` requires transcript hash, URI, verdict hash, resolver decision bond, fraud-window-gated release/slash proof hash, and submission outcome hooks. `agent/resolver.mjs` scans finalized `Challenged` logs, validates canonical Docker transcripts and both instance fingerprints, publishes exact bytes through a wallet-owner-bound content-addressed Arweave adapter, journals the fully signed upload before POST so ambiguous responses replay one transaction ID, journals the validated receipt before policy creation, re-confirms permanence and byte identity through two independent gateways after every restart, journals exact signed calls, and handles receipt reorgs. The current exact-calldata wallet still requires owner provisioning for each dynamic decision. A funded publication credential, live deployment/retrieval rehearsal, reviewed autonomous wallet-policy architecture, signer evidence, and a fraud-proof/equivalent slashing path remain open | Engineering + resolver signers |
...
| No unified production funding authorization | `schemas/production-launch-authorization.schema.json` and `p42-prizes production-launch-authorization-validate` compose exact-byte legal, governance, incident, adversarial, operational, release-verification, completed-deployment, explorer, and ten signed independent math-review packets into one expiring release-bound digest. `p42-funding-activation-plan` emits an exact-ten, runtime-code-pinned plan with global authorize/arm/open barriers. `p42-funding-activate` executes at most one transaction per run, reconstructs every barrier from dual-RPC common-finalized state, reruns validation before each new signature, journals raw bytes before broadcast, and emits a self-hashed completion only after finalized all-open agreement. Contracts bind authorization expiry on-chain. The portal now recomputes the authorization digest, verifies the three Ed25519 authorities against an out-of-band pinned production registry, and publishes no donation target unless all ten completion, manifest, and fresh finalized checkpoint rows agree. This remains source evidence only: no current-deployment rehearsal or real production packet exists, independent signer custody is not approved, and no production artifact generation has passed this path. Until that evidence exists, this gate remains open and no real-ETH funding target is authorized | Engineering + security + counsel + math reviewers |
...
- [x] A required-check CI run passes for exact merged source release `142300ad013b285bcceffbcf3ba527352b43f4ec` ([run 29202286767](https://github.com/techno-optimist/p42-prizes/actions/runs/29202286767)); each later source merge must earn its own successful push run before release.
...
- [x] Source-level resolver runtime validates finalized challenge/transcript bindings and journals/reconciles exact resolver calls (`agent/resolver.mjs`). Local source/test evidence only; this does not attest deployed retrieval, immutable public availability, wallet provisioning, or live resolver operation.
...
| Portal API | Phase 0 routes with schema validation, raw-byte reveal, local JSON persistence, local diagnostic event ledger, process-local rate limits, local idempotency for retryable verifier/submission POSTs, and strict hash-only mutation credentials bound to explicit known route scopes plus optional expiry; malformed/legacy policies fail closed and no wildcard scope exists. Challenges remain honestly unimplemented | Transactional database/event ledger, distributed rate limits/idempotency, provisioned scoped production credentials, audited auth/session policy |
...
- Local verification covers problem validation, certified-path exactness lint, Python tests, seed verification, contracts, agent, web typecheck/tests/build, and package audits. GitHub Actions is published; [`main` run 29202286767](https://github.com/techno-optimist/p42-prizes/actions/runs/29202286767) passed all four lanes for source release `142300ad013b285bcceffbcf3ba527352b43f4ec`. The current Git OAuth credential still cannot revise workflow files; future workflow changes need a workflow-capable owner credential or GitHub connector.
- Governance setup continuation now has a release/config/timelock-bound durable operation journal. Production continuation remains observation-only, consumes the dual-RPC finalized anchor, records primary or deterministic override execution evidence, and reuses the fenced dead-owner-reclaiming journal lock. The 43-contract/110-operation local rehearsal recovers a mined-before-journaled execution and still proves every funding gate false. This is source/rehearsal evidence, not a deployed Gate 1 pass.

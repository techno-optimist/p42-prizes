...
| --- | --- | --- | --- |
| Gate 0: Public repo / local pilot | Mostly green; latest merged source-release CI and live release guard pass, two repository-account controls remain | Local verifier, web/API tests, fail-closed challenge/onramp, live fail-closed PostgreSQL authority, security policy text, published `.github/workflows/ci.yml`, and the schema-validated, canonically sealed [`current source-release receipt`](evidence/source-release-current.json) binding green post-merge [`main` run 29375757945](https://github.com/techno-optimist/p42-prizes/actions/runs/29375757945) plus live Render deploy `dep-d9bc7nt7vvec73a3re60`, both at exact source/deploy commit `b6638ec832e4e9d4bd705811ddf84d1a0a3e5d9f` | Retain a successful push run and release-guard record for each newly merged source release; upgrade the private repository/account tier or make the repository public so protected branch/ruleset enforcement is available; repo owner enables private vulnerability reporting through a supported GitHub surface |
...
- [x] Required-check CI passes for exact source head `b6638ec832e4e9d4bd705811ddf84d1a0a3e5d9f` ([run 29375757945](https://github.com/techno-optimist/p42-prizes/actions/runs/29375757945)); the 10-probe release guard binds the same exact deploy-relevant commit and live deploy `dep-d9bc7nt7vvec73a3re60` in the schema-validated [`current release receipt`](evidence/source-release-current.json). Each later source merge must earn its own successful push run and, when deploy-relevant, a live binding before release.

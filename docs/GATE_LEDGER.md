...
| GitHub Actions workflow | Live source and deploy pass; production receipt blocked on v3 ratification | `.github/workflows/ci.yml` is published. [`main` run 29400163976](https://github.com/techno-optimist/p42-prizes/actions/runs/29400163976) passed all six lanes for exact source head `869839daa047305efde7bf6850a2cfef4abac87a`, including Ubuntu 22.04 and 24.04 SP1 rebuild/replay jobs and their cross-image aggregator. The authenticated Render guard subsequently bound live commit `869839daa047305efde7bf6850a2cfef4abac87a`, runtime commit `e57327efbad1fce38578e667ebeb974f6905b3e2`, the canonical board projection, and 12/12 healthy routes. The prior [`source-release receipt`](evidence/source-release-current.json) remains valid historical v2 evidence for `f6dba23d8d76d4ff833077242397ef71727368db`, not current authorization. V3 source validation now requires PR or threshold-bootstrap authorization for every non-exempt first-parent source commit, keeps Render relevance separate, and rejects approvals submitted after merge; source changes can no longer disappear merely because they do not touch `web/` or `render.yaml` | Establish the externally pinned source-release v3 policy, independently review and threshold-sign every required migration interval, then publish a chained v3 receipt with pinned workflow identity and fresh live probes. Retain a completed push run and release-guard record for every source release. GitHub returned `403` for branch protection/rulesets and `404` for private vulnerability reporting; those are explicit repo-owner/account blockers |
...
- [ ] SP1 candidate-build provenance is fail-closed and forensics-preserving at the source-workflow layer, but exact-head CI has not yet accepted the hardened model. Run `29411110452` rejected both x86 producers before compilation because the Succinct installer normalizes archive metadata; the aggregator then failed closed before consuming validated evidence. The follow-up source binds exact paths, bytes, safe symlink/hardlink topology, and read/execute permission classes while permitting installer-only write-bit normalization; it also uses absolute `/usr/bin/env` and `/usr/bin/python3`, removes cached Git checkouts, fingerprints directory traversal/modes, and records archive and installed trees separately in untrusted observations. This item may return to checked only after an exact-head two-image run passes and the resulting validated provenance bytes match. Candidate and observation artifacts remain explicitly `untrusted`; only post-validation, post-equality artifacts may use `validated` names or enter the cross-image aggregator. The DGX ARM observation remains non-authorizing: official guest archive `sha256:03399c2e31561dcefd58fe1f467b8ef44fc693531f1326520513b3961a7a9267` produced a 101-file sysroot manifest `sha256:425dc79798515e5c8f133c845410313c0c7e57791f6b18e1c187a29b4534ae8b`, but source-built `cargo-prove` `sha256:f0ff2d9ec1e65ced44b5608419f02627c69a74fdcf4466d9f259ebc86bc7dc05` is not an approved release executable and clean ARM candidates drifted (`3bdce30c...` Hadamard, `64bcf886...` A11). Those bytes are counterevidence for host-toolchain drift, not production identity. The canonical frozen hashes above remain unchanged.

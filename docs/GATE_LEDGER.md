...
| --- | --- | --- | --- |
| Gate 0: Public repo / local pilot | Mostly green; latest merged source-release CI and live release guard pass, two repository-account controls remain | Local verifier, web/API tests, fail-closed challenge/onramp, live fail-closed PostgreSQL authority, security policy text, published `.github/workflows/ci.yml`, and [`source-release evidence`](evidence/source-release-2026-07-13-3a3ee37.json) binding green post-merge [`main` run 29243704647](https://github.com/techno-optimist/p42-prizes/actions/runs/29243704647) plus live Render deploy `dep-d9ac2a58nd3s73dbli10` to source release `3a3ee37bb90f98fbdf3c5366a74d24fb4823638e` | Retain a successful push run and release-guard record for each newly merged source release; upgrade the private repository/account tier or make the repository public so protected branch/ruleset enforcement is available; repo owner enables private vulnerability reporting through a supported GitHub surface |
...
| GitHub Actions workflow | Pass for workflow presence, latest merged source-release run, and live deploy binding | `.github/workflows/ci.yml` is published. Post-merge [`main` run 29243704647](https://github.com/techno-optimist/p42-prizes/actions/runs/29243704647) passed all four lanes for exact source release `3a3ee37bb90f98fbdf3c5366a74d24fb4823638e`. The authenticated Render record binds live deploy `dep-d9ac2a58nd3s73dbli10` to that commit, and `make verify-render-release` passed all 8 route/configuration/projection checks. The checked-in [`source-release evidence`](evidence/source-release-2026-07-13-3a3ee37.json) records both authorities and the targeted Atlas/Q7 production probes | Retain a completed push run and release-guard record for every newly merged source release. GitHub returned `403` for branch protection/rulesets and `404` for private vulnerability reporting; those are explicit repo-owner/account blockers |
...
| Canonical 46-contract topology not deployed | `protocol/canonical-topology-v1.json` is the single ordered authority for six shared contracts plus four contracts on each of ten boards. The ceremony now materializes all 46 before signing: direct roots and pool/ledger contracts, twenty factory CREATE2 manager children, and a resolver quorum frozen against the exact ten challenge managers. Manifest, explorer, role-acceptance, indexer, reconciliation, and launch-authorization consumers require the same ordered topology. Factory children carry transaction, event, CREATE2, and historical configuration-getter evidence. Remaining: complete hostile review, merge the atomic migration, run the Base Sepolia ceremony, and independently verify the resulting dossier | Engineering + hostile reviewer |
...
| No unified production funding authorization | `schemas/production-launch-authorization.schema.json` and `p42-prizes production-launch-authorization-validate` compose exact-byte legal, governance, incident, adversarial, operational, release-verification, completed-deployment, explorer, and ten signed independent math-review packets into one expiring release-bound digest. `p42-funding-activation-plan` emits an exact-ten, runtime-code-pinned plan with global authorize/arm/open barriers. `p42-funding-activate` executes at most one transaction per run, reconstructs every barrier from dual-RPC common-finalized state, reruns validation before each new signature, journals raw bytes before broadcast, and emits a self-hashed completion only after finalized all-open agreement. The executor now permits at most one local signer per process and supports exact unsigned-request/external-signed-transaction exchange for independent or HSM-backed signers; legacy threshold-key aggregation variables fail closed. Contracts bind authorization expiry on-chain and now reject replacement of a nonzero authorization through its exact expiry; treasury may issue a new digest only after the prior authorization has expired. The portal recomputes the authorization digest, verifies the three Ed25519 authorities against an out-of-band pinned production registry, and publishes no donation target unless all ten completion, manifest, and fresh finalized checkpoint rows agree. This remains source evidence only: no current-deployment rehearsal or real production packet exists, independent signer custody is not approved, and no production artifact generation has passed this path. Until that evidence exists, this gate remains open and no real-ETH funding target is authorized | Engineering + security + counsel + math reviewers |
...
- [x] Required-check CI and the release guard pass for exact merged source release `3a3ee37bb90f98fbdf3c5366a74d24fb4823638e` ([run 29243704647](https://github.com/techno-optimist/p42-prizes/actions/runs/29243704647), [`release evidence`](evidence/source-release-2026-07-13-3a3ee37.json)); each later source merge must earn its own successful push run and live binding before release.
...
- Governance setup continuation now has a release/config/timelock-bound durable operation journal. Production continuation remains observation-only, consumes the dual-RPC finalized anchor, records primary or deterministic override execution evidence, and reuses the fenced dead-owner-reclaiming journal lock. The 46-contract/110-operation local rehearsal recovers a mined-before-journaled execution and still proves every funding gate false. This is source/rehearsal evidence, not a deployed Gate 1 pass.

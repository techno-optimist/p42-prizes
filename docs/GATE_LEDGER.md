...
- [x] SP1 candidate-build provenance is fail-closed and forensics-preserving at the source-workflow layer. Each x86 matrix lane removes `objective-programs/target`, builds under an empty environment with an explicit minimal `HOME`/`PATH`, rejects Cargo config/compiler/loader overrides, proves the target transitioned from absent to a no-follow in-repository directory, binds the official v6.1 `cargo-prove` archive and executable, binds the `succinct-1.93.0-64bit` guest archive, and compares installed file bytes, modes, and safe symlink/hardlink topology against that archive. An `if: always()` artifact records observed toolchain digests before allowlist validation, and separate `if: always()` artifacts retain candidate ELF/identity/execution bytes before frozen equality; all use explicit `untrusted` names. Only post-validation, post-equality artifacts use `validated` names or enter the cross-image aggregator. The DGX ARM observation remains non-authorizing: official guest archive `sha256:03399c2e31561dcefd58fe1f467b8ef44fc693531f1326520513b3961a7a9267` produced a 101-file sysroot manifest `sha256:425dc79798515e5c8f133c845410313c0c7e57791f6b18e1c187a29b4534ae8b`, but source-built `cargo-prove` `sha256:f0ff2d9ec1e65ced44b5608419f02627c69a74fdcf4466d9f259ebc86bc7dc05` is not an approved release executable and clean ARM candidates drifted (`3bdce30c...` Hadamard, `64bcf886...` A11). Those bytes are counterevidence for host-toolchain drift, not production identity. The canonical frozen hashes above remain unchanged.

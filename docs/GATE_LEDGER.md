...
| --- | --- | --- |
| Contract system incomplete | Local Hardhat 3 scaffold now covers problem registry/freezing, escrow pool, payout ledger, submission bonds, CID-bound commitment helper, commit-time DA hash, reveal, challenge-window-gated finalization, finalize-time permanence hash, ledger credit recording, counter-bond sizing, resolver transcript posting, and 17 invariant tests; still needs challenge/resolver-to-finalization wiring, deployment scripts, fuzzing, and audit | Engineering + external auditor |

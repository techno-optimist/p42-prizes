...
| Gate 1 | Deploy the remediated frozen release under intended testnet roles using only `cd contracts && npm run deploy:base-sepolia`. The canonical npm command unconditionally selects the exact-ten production planner regardless of a caller-supplied mode; direct wrapper invocation rejects missing or wrong mode before importing deployment code. The prior canonical Base Sepolia manifest predates current source, is machine-rejected as stale, and does not attest it. The reviewed ceremony is intentionally two-phase: the deployer durably submits the first 36 transactions and reserves the exact 40-operation predeployment governance journal; independent governance signers execute those bindings; finalized event and receipt evidence must agree across operator-distinct RPCs; only then does the same signed nonce journal resume the remaining 11 deployments before the final 70 setup operations. | New manifest and explorer verification bind the frozen commit, chain ID, all 47 addresses, all 110 governed operations, constructor/wiring/config values, source hashes, and runtime-bytecode hashes; reconciliation runs from deployment genesis. The deployment journal, governance journals, finalized dual-RPC observations, and finality-anchor recheck prove uninterrupted identity across the phase boundary. Test-only `test-only-legacy-p42-prizes.json` output is never accepted. |
| Gate 1 | Retire the historical Base Sepolia pool from commit `3121a1a`. Its public pool `0xCd9B3832ccd0fEeFa331f8544034808d3Fc16c6d` has zero balance but still accepts test ETH because the legacy ledger `0x113bBAf1eFB1a079518B8dF2aCd2590Ba4B41DeF` is open. Public chain history cannot be redacted. | Owner `0xF007fC214139CEA514c712289dfDd9d3a5739F61` calls the legacy ledger `close()`. Retain finalized, operator-distinct dual-RPC evidence that `closed()==true`, the pool remains zero-balance, and both `fund()` and a plain nonzero-value transfer revert with `P42_POOL_CLOSED`. This is testnet retirement evidence only, never authority for the new deployment. |
...
| Gate 1 | Review and deploy the objective verifier fraud-proof tier. The source verifier plus [`sp1-external-runtime-current.json`](evidence/sp1-external-runtime-current.json) currently close only observation of Succinct's pinned v6.1 Groth16 address and finalized runtime on Base and Base Sepolia; the artifact expires after 24 hours and grants no audit authority. | Ten independently reviewed total proof programs; external review of the exact pinned Succinct source/runtime and deployment assumptions; audited immutable P42 gateway bytecode; manifest-bound program/package/runtime-attestation hashes; measured worst-case prover and on-chain verification costs; fraud-window and incentive analysis; fresh dual-operator finalized runtime recapture at activation; and Base Sepolia rehearsals of both corrected outcomes, copied proofs, verifier failure, deadline races, congestion, reorgs, and restart recovery. The mock gateway and a historical runtime observation are never production activation evidence. |
...
- Verifier images: the exact-commit, all-ten OCI release ceremony is implemented,
  ...
  The 2026-07-20 recheck still found no GHCR login in DGX's normal Docker
  credential store and no DGX GitHub CLI session. The active Mac GitHub OAuth
  token now exposes `write:packages` and successfully lists the existing ten
  private verifier repositories, but it also carries broad repository and
  workflow authority and is not installed on DGX. That discovery access does
  not satisfy the release requirement for a narrowly scoped publication
  credential. The owner must provision DGX's normal

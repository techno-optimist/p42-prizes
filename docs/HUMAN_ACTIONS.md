...
| Gate 0 | Maintain CI and live-release evidence. The reviewed workflow is published, and the schema-validated, canonically sealed [`current source-release receipt`](evidence/source-release-current.json) binds post-merge [`main` run 29375757945](https://github.com/techno-optimist/p42-prizes/actions/runs/29375757945), all six required lanes, live Render deploy `dep-d9bc7nt7vvec73a3re60`, exact source/deploy commit `b6638ec832e4e9d4bd705811ddf84d1a0a3e5d9f`, and the 10-probe release guard. | Each source release has a completed required-check run and each deploy-relevant release has a release-guard record against its exact source/deploy lineage. This does not replace protected-release enforcement or private-vulnerability reporting. |

...
| Gate 0 | Maintain CI and live-release evidence. The reviewed workflow is published, and the schema-validated, canonically sealed [`current source-release receipt`](evidence/source-release-current.json) binds post-merge [`main` run 29380606981](https://github.com/techno-optimist/p42-prizes/actions/runs/29380606981), all six required lanes, live Render deploy `dep-d9bdnv741pts73ep1g20`, exact source/deploy commit `c04c37bffdf151bf5ac51b5d3f66bc8e7c9ff164`, and the 10-probe release guard. | Each source release has a completed required-check run and each deploy-relevant release has a release-guard record against its exact source/deploy lineage. This does not replace protected-release enforcement or private-vulnerability reporting. |

...
| Gate 2 | Name governance/custody roles and run the production rehearsal. | Target: `governance-signoff-validate` passes a `p42-governance-signoff/v2` packet; historical v1 is non-authorizing. The v2 packet binds the Base Sepolia chain, exact release capsule, configuration and deployment commits, deployed timelock, and ordered canonical 47-contract topology. EIP-712 possession and explicit risk/role acceptance from timelock signers, guardian, treasury, and resolver-quorum signers; slate binding; and durable `acceptanceValidatedAt` replay are separate future activation evidence. They are not enforced by this validator and must not be inferred from a v2 validation pass. Funding activation independently rejects noncanonical topology. |

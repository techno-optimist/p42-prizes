...
| Gate 0 | Maintain CI and live-release evidence. The reviewed workflow is published, and the schema-validated, canonically sealed [`current source-release receipt`](evidence/source-release-current.json) binds post-merge [`main` run 29384083401](https://github.com/techno-optimist/p42-prizes/actions/runs/29384083401), all six required lanes, source head `f6dba23d8d76d4ff833077242397ef71727368db`, live Render deploy `dep-d9bdnv741pts73ep1g20` at the derived deploy-relevant commit `c04c37bffdf151bf5ac51b5d3f66bc8e7c9ff164`, and the 10-probe release guard. | Each source release has a completed required-check run and each deploy-relevant release has a release-guard record against its exact source/deploy lineage. This does not replace protected-release enforcement or private-vulnerability reporting. |

...
| Gate 1 | Confirm on-chain DA for finalized testnet submissions on the canonical (DA-refactored) deployment. | Reveal-calldata bytes hash to the on-chain `commitDaHash` anchor for on-chain-DA problems (anchored off-chain store fetch + re-hash for the 3 large problems); `p42-prizes da-verify` evidence covers any optional Arweave mirror receipt. See `docs/DATA_AVAILABILITY.md`. |

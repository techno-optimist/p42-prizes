...
| Gate 0 | Maintain CI and live-release evidence. The reviewed workflow is published, and the schema-validated, canonically sealed [`current source-release receipt`](evidence/source-release-current.json) binds post-merge [`main` run 29337383619](https://github.com/techno-optimist/p42-prizes/actions/runs/29337383619) at observed source head `a6e2f83351b64af5d56ee1a2a9c5e239db45391e`, all six required lanes, Render deploy `dep-d9b3nvr7uimc73a8p1tg` at the same deploy-relevant commit, and the 10-probe release guard. | Each source release has a completed required-check run and each deploy-relevant release has a release-guard record against its exact `main` commit. This does not replace protected-release enforcement or private-vulnerability reporting. |
...
- Source control: the CI workflow is published and the current
  ...
  binds successful post-merge [`main` run 29337383619](https://github.com/techno-optimist/p42-prizes/actions/runs/29337383619),
  all six required lanes, and the live Render release to exact source
  `a6e2f83351b64af5d56ee1a2a9c5e239db45391e`. Branch protection and rulesets remain blocked by the private-repository

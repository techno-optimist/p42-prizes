...
| Gate 0 | Maintain CI and live-release evidence. PR #140 is integrated only at candidate head `10f48e8c7951482b0f28dee9c2bdc34320ba6719`, not merged to `main`; at 2026-07-17 05:36 MDT its exact run `29576211886` has portal, autonomous-agent, Python, and contracts green with both SP1 producers pending. PR #141 is evidence-scoped. Live remains `5a4d25d3862d75d2b573e93f33f60ad72662aef7`. | Wait for the exact run's terminal result without predicting success; require the complete seven-job set, merge PR #140, deploy, and capture a new release guard. Separately provision and ratify the v3 trust root. Evidence in PR #141 does not substitute for merge, deploy, or live proof. |
...
| Gate 0 | Enable and test private vulnerability reporting. It is unavailable on the current private repository plan; no enabled state can be claimed. | Repo owner upgrades the applicable plan or makes the repository public, then enables reporting through a supported GitHub security-settings surface and proves a non-owner test reaches maintainers through the advisory route. |
| Gate 0/1 | Complete the portal database cutover. The live database starts with legacy public tables and no `p42_portal` schema. The hardened dual-identity model is candidate source/local evidence only. PR #142 is a draft/source provisioning candidate, not merged, and transfer repair is not merged. | After reviewed source merge, provision the distinct migration-owner/runtime identities and pinned schema, then perform the exact state-transfer ceremony in `PORTAL_DATABASE.md`. Do not substitute the ordinary one-time runtime import. Retain redacted authority and rehearsal evidence and verify live source plus `target: null`. |
| Gate 1 | Provide deployment authority. No Base Sepolia deploy credential and no production governance signer credentials are available in the current environment. | The deployer and independently held governance signers provision credentials out of band, accept their exact roles, and execute the reviewed two-phase ceremony only after the source and release gates close. |
| Gate 1 | Activate funding only after deployment authority closes. Funding is not activated; live targets are `null`, and candidate targets are local-only. | Produce the canonical Base Sepolia deployment, governance, reconciliation, source-release, and activation artifacts before publishing any target. |
...
- Portal checkpoint database upgrade: source now requires a durably pinned
  ...
  The live starting point is legacy public tables with no `p42_portal` schema.
  Dual owner/runtime identities describe the target production design. PR #142
  is draft/source provisioning evidence only; neither provisioning nor transfer
  repair is merged. The exact transfer ceremony is distinct from ordinary import.
...
- Governance/custody: production roles/keys/addresses are unnamed; current
  ...
  No Base Sepolia deploy credential or production governance signer credentials
  are available in the current environment.
- Incident/disclosure: policy remains draft; private reporting is unavailable
  on the current private plan and the mailbox is unverified; no activation
  evidence, completed drill, or required signatures.

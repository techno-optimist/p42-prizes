...
| Gate 0 | Maintain CI evidence. The reviewed workflow is published, and post-merge [`main` run 29216377082](https://github.com/techno-optimist/p42-prizes/actions/runs/29216377082) for exact release `a97b824cbb50c7b4ef1421bc5057c60879397300` passed the Python verifier/seed, contract, autonomous-agent, and portal lanes. | Each deploy-relevant release has a completed required-check run against its exact `main` commit. This does not replace protected-release enforcement or private-vulnerability reporting. |
...
| Gate 2 | Name governance/custody roles and run the production rehearsal. | Target: `governance-signoff-validate` passes, and every timelock signer, guardian, treasury, and resolver-quorum signer supplies an EIP-712 possession plus explicit risk/role acceptance. The current packet schema remains a historical 43-contract rehearsal consumer and is not production authority. It must migrate atomically with the canonical deployer and bind chain ID, exact release/capsule/slate/config/deployment commit, deployed timelock, and the canonical digest of all 46 contracts. Completion records the finalized completion block/timestamp as `acceptanceValidatedAt`; later validation replays signatures at that durable instant rather than wall-clock time. Funding activation independently rejects noncanonical topology throughout this migration. |
...
- Source control: the CI workflow is published and post-merge
  [`main` run 29216377082](https://github.com/techno-optimist/p42-prizes/actions/runs/29216377082)
  for exact release `a97b824cbb50c7b4ef1421bc5057c60879397300` passed all four required

...
| 4 | Resolver committee is the real oracle (can collude to finalize invalid / reject valid) | Critical | Real-ETH target: complete re-run transcript durably available with an on-chain content binding; per-decision bonds slashable by a later fraud proof; no finality until fraud-proof window closes. Current Phase 0 ABI records only transcript hash, URI, and verdict hash. | Committee capture until fraud-proof resolver ships (v2) | **Y** |

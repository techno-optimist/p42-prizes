...
| 9 | Losing/spurious challenge as a timing weapon to delay a rival's finalize | High | Counter-bond scales to delayed value and configured rerun cost. One challenge slot is active at a time; solver-favorable resolution or expiry can reopen a fresh window, but `MAX_CUMULATIVE_DISPUTE_WINDOWS = 3` bounds serial delay. **#1 removes the payout-timing payoff** | A capital-rich attacker can still impose up to the configured cumulative-window bound; testnet timing and liveness evidence remain required | Y |
...
4. **Adjudicate through the current authority boundary.** Public deterministic re-runs produce the evidence, but the bonded EIP-712 quorum posts the authoritative pending decision until objective proofs are active. A valid board-bound SP1 proof can correct that decision during the fraud window; today the gateway is inert. Bond routing is asymmetric and fee-free: an upheld challenge receives the solver posting bond under submission settlement, a losing challenge bond becomes treasury-claimable, and the resolver's separate decision bond follows its own release/slash path. Protocol fees apply only to successful solver payouts.
5. **Resource bounds.** `maxCompute` is an off-chain admission and runner requirement. Contracts do not execute or enforce the verifier budget, and resolver timeout paths can settle without proving a completed re-run. Production therefore requires measured totality, queue/SLA evidence, and active objective-proof authority; the source contract alone does not guarantee adjudication within budget.
...
**Griefing via spurious challenges — HIGH.** *Mitigation:* every challenge posts a **counter-bond** `B_chal`, computed from a floor, delayed entitlement, and configured rerun-cost multiplier. If adjudication rejects the challenge, its bond becomes treasury-claimable; however, `expireChallenge()` returns the full challenge bond when the resolver misses its deadline. A timeout griefer can therefore buy bounded delay with gas and temporary capital rather than a deterministic bond loss. The cumulative-window cap, resolver liveness, and testnet timing evidence are load-bearing.

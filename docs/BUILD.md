...
> **[SPINE OVERRIDE].** The commit preimage is `keccak(answerCID ‖ solverAddr ‖ salt)` — the *CID is inside the preimage* — and the `sha256` content anchor (`commitDaHash`) is bound **at commit**, while the raw solution bytes ride the **reveal** calldata where the contract enforces `sha256(bytes)==commitDaHash` (red-team finding #6 / tech #3). `resolve()` is role-gated behind `onlyResolver` so the v1 verifiable-committee can be swapped for a fraud-proof resolver; **the committee is not trusted-final for real ETH** (tech #2). `PayoutLedger` reserves are *claimable-but-locked until CLOSE/RESOLVED* per mechanism finding #1.

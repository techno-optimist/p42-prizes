...
- **The moat.** An open, exact, deterministic, adversarially-hardened verifier — the **P42 Verifier Standard** ("Deep Thought"). Because the verifier is a pure function returning bit-identical results to every honest re-runner, the on-chain oracle needs no trusted referee. This is the one thing competitors cannot copy without doing the hard verification work.
...
- **Anti-front-running.** **Commit-reveal with the solution CID inside the commit preimage**, and the blob posted to DA at commit time — closing both mempool solution-sniping and the "free-option" grief the red-team found.
- **Hard constraint.** **Exact, deterministic, self-certifiable problems only.** No floating-point scorer, ever — money multiplies verifier-exploit pressure by orders of magnitude.
...
6. **No token at launch.** *Rationale:* a native token is the fastest path to an unregistered-securities offering and invites speculation that corrupts the "pay for verified work" thesis; every mechanism works in ETH/USDC. *Rejected:* utility token / points-with-implied-airdrop. Sustainability instead comes from a **capped protocol fee** (v1: 2.5% on payouts, `MAX_FEE_BPS = 500`) to a Treasury multisig — a service fee, not a distribution.
...
**4. L2 contracts.** A minimal-proxy factory: `ProblemRegistry` (immutable spec/verifier hash once funded) creates one `BountyPool` clone per problem (escrow only, non-upgradeable, pull-payments). `SubmissionManager` (commit-reveal), `ChallengeManager` (optimistic dispute), and `PayoutLedger` (improvement-proportional accounting) sit behind a UUPS proxy governed by a 2-day timelock + 3-of-5 multisig. Funds live under fixed rules regardless of governance; `claim()` can never be frozen.

**5. Optimistic oracle.** The dispute machine: reveal → challenge window → bonded challenge → deterministic re-run → bond forfeiture. v1 resolution is a bonded committee that must post the re-run transcript on-chain; v2 replaces it with a fraud-proof of the deterministic verifier's execution. `resolve()` is role-gated so the swap is a role reassignment for the interface — but a *trust-model* upgrade that gates real-ETH scale.
...
| 4 | Resolver committee is the real oracle (can collude to finalize invalid / reject valid) | Critical | Committee posts full re-run transcript on-chain; per-decision bonds slashable by a later fraud proof; no finality until fraud-proof window closes | Committee capture until fraud-proof resolver ships (v2) | **Y** |
...
| 6 | Front-running a broadcast solution on the public mempool | Critical | Commit-reveal with CID in the commit preimage; blob to DA at commit time; salt-only reveal | Commit-time censorship/delay → windows sized generously; L1 force-include (v2) | **Y** |
| 7 | Data unavailable for later Δ recomputation (prior `v*` blob expired) | High | Mandatory Arweave permanence receipt verified on-chain at finalize; store Arweave txid, not just CID; fail-closed on unretrievable data | Arweave cost per finalize; permanence-provider risk | **Y** |
...
| 13 | Upgrade / admin-key compromise drains pools or swaps verifier | Critical | Immutable fund-holding pools; upgrades gated by multisig + 48h timelock; verifier registration append-only; `claim()` unfreezable | Multisig collusion — timelock is the exit backstop | Y |
...
- **Phase 2 — Audit + legal + trust-root upgrade.** External smart-contract audit (pools, bonds, resolver, AA/Paymaster). Written legal opinion on the bounty/prize framing, money-transmission, and KYC/sanctions posture. Stand up the *verifiable resolver* (on-chain transcript + bonded committee) as the minimum acceptable real-ETH oracle. **Gate:** clean audit + written legal sign-off + resolver posts verifiable transcripts on testnet.
...
**Conflict-resolution note.** Where sections disagreed, this spine is authoritative and states the resolution: (i) payout **does not stream live** — the mechanism section's linear-vesting-with-live-release is *overridden* by escrow-until-close + `min(vested, final entitlement)` per red-team finding #1; (ii) the bond formula uses `pool_at_submission`, not a provisional share denominator, per finding #5; (iii) commit-reveal puts the **CID in the preimage** with DA at commit time, superseding the answer-hash-only variant; (iv) determinism admission is the **N-host matrix**, superseding "two runs on two hosts"; (v) DA requires an **Arweave permanence receipt at finalize**, superseding "IPFS pin + `T_chal < blobRetention`." The v1 resolver committee is explicitly **not** trusted-final for real ETH.
...
function commit(uint256 problemId, bytes32 commitHash) external payable returns (uint256 subId); // pays postingBond; blob to DA at commit
function reveal(uint256 subId, string calldata answerCID, int256 claimedScore, bytes32 salt) external;
...
function challenge(uint256 subId, bytes32 reasonHash) external payable; // pays challengeBond
function finalize(uint256 subId) external;         // after window, no live challenge
function resolve(uint256 subId, bool challengerWins, bytes calldata verdict) external onlyResolver;
event Challenged(uint256 indexed subId, address indexed challenger, uint64 disputeEnds);
event Resolved(uint256 indexed subId, bool challengerWins);
...
```
        commit(bond) ──► COMMITTED ──reveal (before commitDeadline)──► REVEALED
                          ...
                          │                                                │
   REVEALED ──challenge(bond)──► CHALLENGED ──resolve──► ┌ challengerWins ► REJECTED (solver bond → challenger + pool)
                                                          └ solverWins ───► FINALIZED (challenger bond → solver)
```

Commit→reveal gap is bounded (`commitDeadline = commit + 1h`): the commit hides the answer CID and score so a watcher cannot copy a pending winning solution from the mempool and front-run the reveal. Only after reveal is the answer public and the `challengeWindow` (v1 = 48–72h) opens.
...
- **Upgradeability:** logic contracts behind a **UUPS proxy** governed by a **2-day timelock + 3-of-5 multisig**; `BountyPool` clones are **non-upgradeable and immutable** — user funds live under fixed rules regardless of governance. A `Pause` guardian can halt *new* commits/challenges but can **never** freeze `claim()` (funds are always withdrawable). Minimizing trust: registry `specHash`/verifier metadata is immutable once a pool is funded (`frozen=true`).
...
- **Data availability:** answer blobs (matrices, vectors) live on **IPFS pinned + Arweave for permanence**; only the CID (32–46 bytes) and `claimedScore` go on-chain. Challengers must be able to fetch the blob — a submission whose CID is unretrievable is auto-rejected at challenge (unavailability = invalid). **[SPINE OVERRIDE — tech #4]:** because `Δ` is computed relative to the prior best, prior winning blobs must remain available for the *problem's whole life*; finalize therefore requires a verified **Arweave permanence txid**, stored on-chain, not merely an IPFS pin with `T_chal < blobRetention`.
...
- **R3 — Determinism & reproducibility.** Same input bytes → same output bytes, on any admissible host. Concretely: pinned dependency lockfile (hashes, not ranges), a content-addressed container image (`sha256` digest, not a mutable tag), no wall-clock/RNG/network/filesystem reads on the certified path, fixed integer/float environment (no `-ffast-math`, no locale-dependent parsing), stable serialization (canonical JSON: sorted keys, no insignificant whitespace, integers as strings when they exceed 2^53). **[SPINE OVERRIDE — tech #1]:** enforce structurally — `PYTHONHASHSEED=0`, `OMP_NUM_THREADS=1`, single-thread all BLAS, AST-lint the verifier to reject any `float`/`math.`/numpy-float-dtype on the certified path, require canonical-sorted iteration, and **gate admission on an N-host matrix (x86 + ARM + two glibc versions) hashing identically** — not two runs on two similar hosts.
...
**Solution disappears before a challenger can fetch it — HIGH.** *Mitigation:* the reveal must include the **full solution bytes on a guaranteed-availability layer for the challenge window** — v1 posts the solution as **L2 blob (EIP-4844)** plus an IPFS mirror; a submission whose data is not retrievable at reveal is **invalid by default** (fail-closed). **[SPINE OVERRIDE — tech #4]:** because later `Δ` recomputes against prior bests, a **mandatory Arweave permanence receipt** (on-chain txid) is required at finalize, not just `T_chal < blobRetention`. *Residual:* Arweave cost per finalize; permanence-provider risk.
...
5. **Immutable/timelock-multisig pool contracts + permanent (Arweave) fail-closed data availability (§e, §f):** funds cannot be rugged by an admin key or stranded by vanishing solution data.
...
Every bounty is one GitHub repo (mirrored to IPFS/Arweave) conforming to a versioned template. Curation gate: a problem is admissible **only if its verifier is self-certifiable, exact, and deterministic** — integer/rational/symbolic arithmetic, no floating-point scorer, ever. Required layout:
...
- **IPFS/Arweave pinning:** submissions and repo snapshots pinned to IPFS (web3.storage) with an **Arweave permanent mirror** for anything that ever touches a payout, so a solved problem's evidence is immutable; on-chain we store the CID + Arweave txid.

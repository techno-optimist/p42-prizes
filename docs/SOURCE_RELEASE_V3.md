...
4. Every first-parent commit in the predecessor-to-observed interval requires authorization, regardless of whether it changes contracts, verifier source, workflows, problems, schemas, deployment source, or another tracked path. The sole ordinary exemption is a commit with a nonempty changed-path set entirely contained in the threshold-signed policy's `evidenceOnlyPaths`. Empty commits and mixed evidence/source commits still require authorization.
5. Online validation authenticates remote `main` before applying the activation/downgrade rule, then authenticates the exact GitHub run attempt/jobs, merged PR and PR-head tree, allowlisted non-author approvals at that exact head and submitted unambiguously before the authenticated merge time, canonical Render deployment, and evidence-only publication tail. Equal timestamps fail closed because GitHub timestamps do not establish ordering within the second. It pins and executes the committed 12-probe guard plus its board manifest, compares the guard's exported routes to the signed policy, and independently binds marker/API/final-URL/equivalence observations.
...
- `deployProvenance`: predecessor baseline and an ordered, complete list of every authorization-required first-parent source commit with first parent, tree, changed-path digest, and PR or bootstrap authorization. The historical field name is retained for v3 compatibility; its scope is not limited to Render-deploying paths.

  ...
  for (;;) {
    try {
      ...
      if (decision.action === "reveal") {
        ...
        await sendPersistent(label, () => chal.expireChallenge.populateTransaction(submissionId));
      } else if (decision.action === "finalize_resolution") {
        const label = `finalizeResolution:${snapshot.resolverBondReleaseAt}`;
        await sendPersistent(label, () => chal.finalizeResolution.populateTransaction(submissionId));

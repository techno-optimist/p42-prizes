...
export async function loadProductionValidationContext(manifest, { env = process.env, provider = null, slatePath = null, capsulePath = null, dossierPath = null, dossierDigest = null } = {}) {
  if (manifest?.releaseMode !== "production") return {};
  ...
  await Promise.all([...new Set(contractEntries(manifest).map(({ blockNumber }) => blockNumber))].map(async (blockNumber) => {
    const block = await provider.getBlock(blockNumber);
    const entries = contractEntries(manifest).filter((entry) => entry.blockNumber === blockNumber);
    if (!block || !Number.isSafeInteger(block.timestamp) || !/^0x[0-9a-fA-F]{64}$/.test(String(block.hash))) throw new Error(`provider did not return canonical deployment block ${blockNumber}`);
  ...
  return { ...context, blockTimestampResolver: ({ blockNumber }) => timestamps.get(blockNumber) };
}

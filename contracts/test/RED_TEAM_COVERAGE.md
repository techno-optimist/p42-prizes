...
| 7 | DA/permanence unavailable for later recompute | Partial | On-chain mechanism covered (post-DA-refactor semantics): for `onchainDa=true` problems the raw solution bytes ride the reveal calldata and the contract enforces `sha256(bytes) == commitDaHash` — `accepts a reveal whose bytes hash to the anchor, recoverable from calldata`, `rejects reveal bytes that do not hash to the anchor (fraud caught on-chain)`, `rejects reveal bytes above the per-problem cap (calldata-bomb guard)` (`p42-da.test.js`); off-chain-DA problems stay gated by the same `commitDaHash` anchor (`accepts an empty reveal; the anchor still binds the off-chain bytes`). The former finalize-time Arweave receipt gate (`P42_EMPTY_PERMANENCE_HASH`) is **removed**: `finalize` accepts `permanenceHash = bytes32(0)`; the hash is an optional mirror record only (`finalizes with a zero permanence hash (Arweave receipt no longer required)`). Finalize grace before close: `blocks ledger close while revealed submissions can still finalize permanence` (gate1). Long-horizon retrievability beyond the blob window is off-chain (Gate 2). |

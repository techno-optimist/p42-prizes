...
| 9 | Spurious challenge as a timing weapon | Yes | H2: `sizes counter-bonds from the ledger-derived disputed entitlement, not a caller arg` (gate1). M1: `rejects an unadjudicated submission on resolver timeout so close can proceed safely` proves that resolver outage creates no credit and no pool lock; `f3: resolver timeout cannot turn an unverified submission into payable credit` proves each party recovers only its own bond. A quorum-decided solver win may re-arm only within the cumulative deadline. Residual timeout censorship remains an operational Gate 2 risk. |

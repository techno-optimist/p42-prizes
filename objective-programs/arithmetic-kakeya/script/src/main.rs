use p42_arithmetic_kakeya_objective_core::{
    sha256, verify_arithmetic_kakeya, verify_arithmetic_kakeya_and_journal, word_u128,
    ObjectiveWitness,
};
use sp1_sdk::{
...
const ELF: Elf = include_elf!("p42-arithmetic-kakeya-objective-program");
...
fn run() -> Result<(), String> {
    ...
    match args.next().as_deref() {
        ...
        Some("identity-elf") => {
            ...
            if args.next().is_some() { return Err("usage: ... identity-elf PROGRAM.elf".into()); }
            print_identity(fs::read(path).map_err(|error| error.to_string())?.into())
        }
        Some("execute-fixture") => {
            let path = args.next().ok_or("usage: ... execute-fixture SOLUTION.json")?;
            if args.next().is_some() { return Err("usage: ... execute-fixture SOLUTION.json".into()); }
            execute_fixture(ELF, fs::read(path).map_err(|error| error.to_string())?)
        }
        Some("execute-fixture-elf") => {
            let elf = args.next().ok_or("usage: ... execute-fixture-elf PROGRAM.elf SOLUTION.json")?;
            let solution = args.next().ok_or("usage: ... execute-fixture-elf PROGRAM.elf SOLUTION.json")?;
            if args.next().is_some() { return Err("usage: ... execute-fixture-elf PROGRAM.elf SOLUTION.json".into()); }
            execute_fixture(fs::read(elf).map_err(|error| error.to_string())?.into(), fs::read(solution).map_err(|error| error.to_string())?)
        }
        _ => Err("usage: p42-arithmetic-kakeya-objective-script identity | execute-fixture SOLUTION.json".into()),
    }
}
...
fn print_identity(elf: Elf) -> Result<(), String> {
    ...
    println!("{{\"schema\":\"p42-objective-program-identity/v1\",\"guestElfSha256\":\"0x{}\",\"programVKey\":\"{}\",\"publicValuesBytes\":32,\"sp1Version\":\"6.1.0\"}}", hex::encode(sha256(&elf)), pk.verifying_key().bytes32());
    Ok(())
}

fn execute_fixture(elf: Elf, solution: Vec<u8>) -> Result<(), String> {
    let score = verify_arithmetic_kakeya(&solution).ok_or("invalid arithmetic-kakeya fixture")?;
    let prover = MockProver::new();
    let pk = prover
        .setup(elf.clone())
        .map_err(|error| error.to_string())?;
    ...
    let expected =
        verify_arithmetic_kakeya_and_journal(&witness).map_err(|error| error.to_string())?;

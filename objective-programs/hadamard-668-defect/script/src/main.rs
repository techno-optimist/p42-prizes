use p42_objective_core::{
    sha256, verify_hadamard_668_and_journal, word_u128, ObjectiveWitness, SCORE_ATOM_SCALE,
};
use sp1_sdk::{
...
const ELF: Elf = include_elf!("p42-hadamard-668-objective-program");
...
fn run() -> Result<(), String> {
    ...
    match args.next().as_deref() {
        ...
        _ => Err(
            "usage: p42-hadamard-668-objective-script identity | execute WITNESS.json | execute-fixture SOLUTION.json"
                .into(),
        ),
    }
}
...
fn execute_witness(
    ...
    let expected = verify_hadamard_668_and_journal(&witness).map_err(|error| error.to_string())?;

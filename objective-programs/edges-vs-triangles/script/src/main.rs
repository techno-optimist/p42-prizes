use p42_edges_objective_core::{
    sha256, verify_edges_vs_triangles, verify_edges_vs_triangles_and_journal, word_i128, word_u128,
    ObjectiveWitness,
};
use sp1_sdk::{
...
const ELF: Elf = include_elf!("p42-edges-objective-program");
...
fn run() -> Result<(), String> {
    ...
    match args.next().as_deref() {
        ...
        _ => {
            Err("usage: p42-edges-objective-script identity | execute-fixture SOLUTION.json".into())
        }
    }
}
...
fn execute_fixture(elf: Elf, solution: Vec<u8>) -> Result<(), String> {
    let score = verify_edges_vs_triangles(&solution).ok_or("invalid edges fixture")?;
    let prover = MockProver::new();
    let pk = prover
        .setup(elf.clone())
        .map_err(|error| error.to_string())?;
    ...
    let expected =
        verify_edges_vs_triangles_and_journal(&witness).map_err(|error| error.to_string())?;

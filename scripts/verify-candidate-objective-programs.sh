...
cargo=("$cargo_bin" "+$rust_toolchain")
"${cargo[@]}" --version | grep -q '^cargo 1\.91\.1 '
case "${1:-}" in
  "") reproduction_verifier="$root/scripts/verify-sp1-objective-reproduction.py" ;;
  --host-observation) reproduction_verifier="$root/scripts/verify-sp1-host-observation.py" ;;
  *) echo "usage: $0 [--host-observation]" >&2; exit 2 ;;
esac
test "$#" -le 1
